...
int main(int argc, const char *argv[])
{
    ...
    if (!showHelp)
    {
        ...
        for (int arg = 0; arg < argc; ++arg)
            if (argv[arg][0] != '-') target = atoi(argv[arg]);
    	...
    	if (target)
    	{
            ...
            if (GetPidForTid(target) == target)
            {
                ...
                if (restart) TaskGetIdentifier(target, procname);
            }
            else strcpy(tasktype, "Thread");
            ...
            switch (Kill(target))
            {
                case KILL_STATUS_SUCCESSFUL:
                {
                    ...
                    else if (restart && !procname[0]) fprintf(stderr, "unable to restart a Thread\n");
                }
                break;

                case KILL_STATUS_NOT_FOUND: fprintf(stderr, "%s %d does not exist\n", tasktype, target); break;
                default: fprintf(stderr, "Please supply a valid task id.\n"); break;
            }
        }
    }
