 ...
 * compare by id
 */
static inline bool compareByPid(const ProcessDescriptor &task1, const ProcessDescriptor &task2) { return task1.main.id > task2.main.id; }

/*
 * compare by Name
 */
static inline bool compareByName(const ProcessDescriptor &task1, const ProcessDescriptor &task2) { return strcmp(task1.main.identifier, task2.main.identifier) > 0; }

/*
 * compare by Memory
 */
static inline bool compareByMem(const ProcessDescriptor &task1, const ProcessDescriptor &task2) { return task1.heapUsed > task2.heapUsed; }
...
void showproc(Compare compare, bool useMegabytes)
{
	...
	for (llist<ProcessDescriptor>::iterator it = proclist.begin(); it != proclist.end(); it++)
	{
		...
		//}
		println("%5d %6d %6d %-18s %-38s", data.main.id, data.heapUsed, data.imageSize / 1024, data.main.identifier, data.sourcePath);
	}
}
...
int main(int argc, const char *argv[])
{
	...
	static bool (*compare)(const ProcessDescriptor &task1, const ProcessDescriptor &task2) = &compareByPid;
	...
	// do a mode
	if (!showhelp) showproc(compare, useMegabytes);
	else usage(argv[0]);
