...
void Parser::step()
{
	if (position < input.size()) current = input[position++];
	else current = -1;
}
...
bool Parser::pipeExpression(PipeExpression **out)
{
	...
	while (true)
	{
		...
		{
			std::cout << (char) 27 << "[31m";
			std::cerr << "syntax error: expected a program call at position: " << position << std::endl;
			std::cout << (char) 27 << "[0m";
			return false;
		}
	}
...
bool Parser::argument(std::string out)
{
	...
	while (true)
	{
		...
		{
			if (current == '"' && !escaped) instr = false;
			else  arg += current;
			if (escaped) escaped = false;
		}
		else
		{
			if (!escaped && (current == '|' || current == ' ')) break;
			else if (current == '"' && !escaped) instr = true;
			else arg += current;
			if (escaped) escaped = false;
		}
