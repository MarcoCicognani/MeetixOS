 ...
 *
 */
bool createPipe(Pid thisPid, Pid requesterPid, Pid targetPid, File_t source, File_t *out, File_t target)
{
...
bool findFile(const char *path, File_t *file)
{
	...
	if ((*file = OpenF(path, O_RDONLY)) != FD_NONE) return true;
	...
	for (std::string dir : paths)
	{
		...
		if (dir == "app") dir += std::string(path) + std::string("/bin");

		// try open the file
		if ((*file = OpenF((dir + std::string(path)).c_str(), O_RDONLY)) != FD_NONE) return true;
	}
