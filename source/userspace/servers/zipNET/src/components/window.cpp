...
void Window_t::paint()
{
	...
	cairo_close_path(cr);
	cairo_set_source_rgba(cr, _red(shapeColor), _green(shapeColor), _blue(shapeColor), focused ? 0.8 : 0.5);
	cairo_fill(cr);
	...
	if (crossHovered || crossPressed)
	{
		...
		cairoRoundedRectangle(cr, crossBounds.x, crossBounds.y, crossBounds.width, crossBounds.height, radius);
		cairo_fill(cr);
	}

	// draw cross
	int crossPadding = 3;
	if (crossHovered || crossPressed) cairo_set_source_rgba(cr, 0, 0, 0, 255);
	else cairo_set_source_rgba(cr, 200, 0, 0, 255);
	...
	if (resizable)
	{
		...
		if (maximizeHovered || maximizePressed)
		{
			...
			cairoRoundedRectangle(cr, maximizeBounds.x - 3, maximizeBounds.y - 3, maximizeBounds.width + 6, maximizeBounds.height + 6, radius);
			cairo_fill(cr);
		}

		// draw rectangle
		if (maximizeHovered || maximizePressed) cairo_set_source_rgba(cr, 0, 0, 0, 255);
		else cairo_set_source_rgba(cr, 0, 200, 0, 255);
