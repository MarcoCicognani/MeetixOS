...
int main(int argc, char* argv[])
{
	...
	{
		if (!strcmp("--headless", argv[i])) headless = true;
	}
...
void CandyTerminal::execute()
{
	...
	// start shell
	startShell();
	if (shellIN == FD_NONE || shellOUT == FD_NONE || shellERR == FD_NONE) return;
...
void CandyTerminal::initializeScreen()
{
	...
	if (guiScreen->initialize()) screen = guiScreen;

	else Utils::log("CandyTerminal: Failed to initialize the graphical screen");
}
...
void CandyTerminal::startShell()
{
	...
	SpawnStatus status = SpawnPOI("/cmd/mx", "-sh", "/", SECURITY_LEVEL_APPLICATION, &outPid, stdioTarget, stdioIN);
...
void CandyTerminal::inputRoutine()
{
	...
	while (true)
	{
		...
		if (inputMode == SHELL_MODE_DEFAULT)
		{
			...
			{
				if (currentProcess) RaiseSignal(currentProcess, SIGINT);
			}
		...
		else if (inputMode == SHELL_MODE_RAW)
		{
			...
			else if (readInput.key == "KEY_ARROW_LEFT" && readInput.pressed) writeShellkeyToShell(SHELLKEY_LEFT);
			else if (readInput.key == "KEY_ARROW_RIGHT" && readInput.pressed) writeShellkeyToShell(SHELLKEY_RIGHT);
			else if (readInput.key == "KEY_ARROW_UP" && readInput.pressed) writeShellkeyToShell(SHELLKEY_UP);
			else if (readInput.key == "KEY_ARROW_DOWN" && readInput.pressed) writeShellkeyToShell(SHELLKEY_DOWN);
...
void CandyTerminal::outputRoutine(OutputRoutineStartinfo *info)
{
	...
	{
		FsReadStatus stat;
		int r = ReadS(info->errorOutput ? info->terminal->shellERR : info->terminal->shellOUT, buffer, buflen, &stat);
...
void CandyTerminal::processOutputCharacter(StreamControlStatus *status, bool errorStream, char c)
{
	if (status->status == TERMINAL_STREAM_STATUS_TEXT)
	{
		...
		else if (c == 27 /* ESC */) status->status = TERMINAL_STREAM_STATUS_LAST_WAS_ESC;

		else
		{
			int fg = screen->getColorForeground();
			if (errorStream) screen->setColorForeground(SC_RED);

			screen->writeChar(c);

			if (errorStream) screen->setColorForeground(fg);
		}
		// ESC was sent
	}
	...
	else if (status->status == TERMINAL_STREAM_STATUS_LAST_WAS_ESC)
	{
		...
		if (c == '[') status->status = TERMINAL_STREAM_STATUS_WITHIN_VT100;

		// otherwise reset
		else if (c == '{') status->status = TERMINAL_STREAM_STATUS_WITHIN_TERM;

		else status->status = TERMINAL_STREAM_STATUS_TEXT;

		// Handle sequence parameters
	}

	else if (status->status == TERMINAL_STREAM_STATUS_WITHIN_VT100 || status->status == TERMINAL_STREAM_STATUS_WITHIN_TERM)
	{
		// Parameter value
		if (c >= '0' && c <= '9')
		{
			if (status->parameterCount == 0) status->parameterCount = 1;
		...
		else
		{
			...
			if (status->status == TERMINAL_STREAM_STATUS_WITHIN_VT100) processVT100Sequence(status);

			else if (status->status == TERMINAL_STREAM_STATUS_WITHIN_TERM) processTermSequence(status);
...
void CandyTerminal::processVT100Sequence(StreamControlStatus *status)
{
	...
	case 'n':
		...
		if (status->parameters[0] == 6)
		{
			...
			response << (char) SHELLKEY_ESC << "[" << screen->getCursorY() << ";" << screen->getCursorX() << "R";
	...
	// Set scroll area
	case 'r':
		if(status->parameterCount == 0) screen->setScrollAreaScreen();
		else screen->setScrollArea(status->parameters[0], status->parameters[1]);
		break;
...
void CandyTerminal::processTermSequence(StreamControlStatus *status)
{
	...
	case 'i': {
		std::stringstream response;
		response << (char) SHELLKEY_ESC << "{" << screen->getWidth() << ";" << screen->getHeight() << "i";
...
ScreenColor CandyTerminal::convertVT100ToScreenColor(int color)
{
	switch (color)
	{
		case VT100_COLOR_BLACK:   return SC_BLACK;
		case VT100_COLOR_BLUE:    return SC_BLUE;
		case VT100_COLOR_CYAN:    return SC_CYAN;
		case VT100_COLOR_GREEN:   return SC_GREEN;
		case VT100_COLOR_MAGENTA: return SC_MAGENTA;
		case VT100_COLOR_RED: 	  return SC_RED;
		case VT100_COLOR_WHITE:   return SC_WHITE;
		case VT100_COLOR_YELLOW:  return SC_YELLOW;
	}
	return SC_WHITE;
}
