...
void EvaKernel::preSetup(SetupInformation *info)
{
	...
	if (!ramDiskModule) panic("%! ramdisk MXfs.img does not exist", "Eva Kernel");
	...
	logInfo("%! %iMB avaible for the system, %iMB used by ramdisk", "Eva Kernel", avbmm / 1024, rmdiskSize / 1024);
}
...
uint32_t EvaKernel::loadRamdisk(MultibootModule *ramdiskModule)
{
	...
	if (!ramdiskNewLocation) panic("%! not enough virtual space for ramdisk remapping", "Eva Kernel");
...
void EvaKernel::loadSystemProcess(const char *binaryPath, ThreadPriority priority)
{
	...
	switch (status)
	{
		case Elf32SpawnStatus::SUCCESSFUL: logInfo("%! successful spawned \"%s\"", "sysproc", binaryPath); break;
		case Elf32SpawnStatus::FILE_NOT_FOUND: panic("%! \"%s\" not found", "Eva Kernel", binaryPath); break;
		case Elf32SpawnStatus::VALIDATION_ERROR: panic("%! \"%s\" is not a valid elf32 binary", "Eva Kernel", binaryPath); break;
		case Elf32SpawnStatus::PROCESS_CREATION_FAILED: panic("%! \"%s\" could not be loaded, error creating process", "Eva Kernel", binaryPath); break;
