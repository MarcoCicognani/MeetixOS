...
RsdpDescriptor *RsdpLookupUtil::findRSDP()
{
	...
	rsdp = findRSDPinEBDA();
	if (!rsdp) rsdp = findRSDPinRange(0x000E0000, 0x000FFFFF);
	...
	if (rsdp && validateRSDP(rsdp))
	{
		...
		{
			case 0: logDebugn("1.0"); break;
			case 1: logDebugn("2.0"); break;
			case 2: logDebugn("3.0"); break;
			default: logDebugn("? (revision %i)", rsdp->revision); break;
		}
		logDebug("");
