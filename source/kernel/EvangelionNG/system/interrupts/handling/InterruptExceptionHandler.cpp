...
Thread *InterruptExceptionHandler::handleGeneralProtectionFault(Thread *currentThread)
{
	...
	if (currentThread->type == THREAD_TYPE_VM86)
	{
		...
		{
			// success
			case VMResult::SUCCESSFUL: return currentThread;
	...
	dump(currentThread);
	logInfo("%! #%i process %i killed due to general protection fault", "exception", System::currentProcessorId(), currentThread->process->main->id);
	return Tasking::schedule();
}
...
Thread *InterruptExceptionHandler::handlePageFault(Thread *currentThread)
{
	...
	if (accessedVirtual >= currentThread->process->imageStart && accessedVirtual <= currentThread->process->imageEnd)
	{
		...
		if (table[pi] != 0)
		{
			...
			if (!PPreferenceTracker::decrement(accessedPhysical)) PPallocator::free(accessedPhysical);
...
Thread *InterruptExceptionHandler::handleInvalidOperationCode(Thread *currentThread)
{
	...
	currentThread->process->main->alive = false;
	currentThread->alive = false;
	logInfo("%! #%i process %i killed due to invalid operation code %h in thread %i", "exception", System::currentProcessorId(), currentThread->process->main->id, *((uint8_t* ) currentThread->cpuState->eip), currentThread->id);
