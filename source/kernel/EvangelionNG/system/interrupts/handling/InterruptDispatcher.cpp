...
ProcessorState *InterruptDispatcher::handle(ProcessorState *cpuState)
{
	...
	if (cpuState->intr < 0x20) currentThread = InterruptExceptionHandler::handle(currentThread);
	else currentThread = InterruptRequestHandler::handle(currentThread);

	// sanity check
	if (currentThread->waitManager != 0) EvaKernel::panic("scheduled thread %i had a wait manager ('%s')", currentThread->id, currentThread->waitManager->debugName());
