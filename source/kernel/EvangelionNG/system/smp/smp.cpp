...
void Smp::initializeCore(Processor *cpu)
{
	...
	// Send SIPI
	Lapic::write(APIC_REGISTER_INT_COMMAND_HIGH, cpu->apic << 24);
	Lapic::write(APIC_REGISTER_INT_COMMAND_LOW, vectorValue | APIC_ICR_DELMOD_SIPI | APIC_ICR_LEVEL_ASSERT);
...
void Smp::initialize(PhysicalAddress initialPageDirectoryPhysical)
{
	...
	*((uint32_t*) CONST_SMP_STARTUP_AREA_AP_START_ADDRESS) = (VirtualAddress) EvaKernel::runAdvancedSystemPackage;
	*((uint32_t*) CONST_SMP_STARTUP_AREA_AP_COUNTER) = 0;

	logDebug("%! initial page directory for APs: %h", "smp", *((uint32_t*) CONST_SMP_STARTUP_AREA_PAGE_DIRECTORY));
	logDebug("%! kernel entry point for APs: %h", "smp", *((uint32_t*) CONST_SMP_STARTUP_AREA_AP_START_ADDRESS));
	logDebug("%! initial AP counter value: %i", "smp", *((uint32_t*) CONST_SMP_STARTUP_AREA_AP_COUNTER));
	...
	for (uint32_t i = 0; i < System::getNumberOfProcessors(); i++)
	{
		...
		AddressSpace::map(stackVirtual, stackPhysical, DEFAULT_KERNEL_TABLE_FLAGS, DEFAULT_KERNEL_PAGE_FLAGS);

		VirtualAddress stackTop = (stackVirtual + PAGE_SIZE);
		stackArray[i] = stackTop;

		logDebug("%! created AdvancedPackage stack (%h) placed at %h", "smp", stackArray[i], &stackArray[i]);
	}

	// Copy start object from ramdisk to lower memory
	const char *smpstartPath = "/sys/eva/module/smpstart.eko";
	RamdiskEntry *startupObject = EvaKernel::ramdisk->findAbsolute(smpstartPath);
	if (!startupObject)
	{
		logInfo("%*%! could not initialize due to missing smpstart object at '%s'", 0x0C, "smp", smpstartPath);
		return;
	}
	Memory::copy((uint8_t*) CONST_SMP_STARTUP_AREA_CODE_START, (uint8_t*) startupObject->data, startupObject->datalength);

	// Start APs
	Processor *current = System::getProcessorList();
	while (current)
	{
		if (!current->bsp) initializeCore(current);
