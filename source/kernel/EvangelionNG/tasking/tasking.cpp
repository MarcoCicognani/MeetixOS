...
Thread *Tasking::fork(Thread *currentThread)
{
	...
	else logInfo("%! tryed to fork a non main thread", "tasking");
...
void Tasking::addTask(Thread *task, bool enforceCurrentCore)
{
	...
	// Error check
	if (!target) EvaKernel::panic("%! couldn't find scheduler to add task to", "tasking");
...
Scheduler *Tasking::currentScheduler()
{
	...
	// Error check
	if (!scheduler) EvaKernel::panic("%! no scheduler exists for core %i", "tasking", coreID);
...
void Tasking::removeThreads(Process *process)
{
	...
	if (process->isServer) servers->erase(process);
...
Process *Tasking::getServer(const char *identifier)
{
	// find if there is another server with the same name
	for (llist<Process*>::iterator it = servers->begin(); it != servers->end(); ++it)
		if (String::equals(identifier, (*it)->main->getIdentifier())) return *it;
 ...
 * @param firstExist:				the first existing thread with same name that we shall set
 * @param preferredIdentifier:		identifier to adjust and set to target
 * @return the setupped identifier for the target
 */
const char *Tasking::adjustTaskIdentifier(Thread *target, Thread *firstExist, const char *preferredIdentifier)
{
 ...
 */
bool Tasking::registerTaskForIdentifier(Thread *task, const char *newIdentifier, char *selectedIdentifier)
{
	...
	if (*completeName != '\0')
	{
		...
		{
			// show warning
			logWarn("%! task %i could not be registered as '%s', auto adding a supplementary numeric id", "tasking", task->id, newIdentifier);

			// we have to adjust the name
			if (selectedIdentifier) String::copy(selectedIdentifier, adjustTaskIdentifier(task, existing, newIdentifier));
			return false;
		}
	}*/
