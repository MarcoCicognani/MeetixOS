...
public:
	...
	virtual FsTransactionHandlerFinishStatus afterFinishTransaction(Thread *thread)
	{
		...
		else if (status == FS_DISCOVERY_NOT_FOUND) data()->status = FS_OPEN_DIRECTORY_NOT_FOUND;
		else if (status == FS_DISCOVERY_ERROR || status == FS_DISCOVERY_BUSY) data()->status = FS_OPEN_DIRECTORY_ERROR;
