...
FsNode *FsDelegateRamdisk::createVfsNode(RamdiskEntry *ramdiskNode, FsNode *parent)
{
	...
	if (ramdiskNode->type == RAMDISK_ENTRY_TYPE_FILE) node->type = FS_NODE_TYPE_FILE;
	else node->type = FS_NODE_TYPE_FOLDER;
 ...
 *
 */
FsTransactionID FsDelegateRamdisk::requestDiscovery(Thread *requester, FsNode *parent, char *child, FsTransactionHandlerDiscovery *handler)
{
	...
	if (parent->type == FS_NODE_TYPE_MOUNTPOINT) ramdiskParent = EvaKernel::ramdisk->getRoot();
	else ramdiskParent = EvaKernel::ramdisk->findById(parent->physFsID);
	...
	if (ramdiskParent)
	{
		...
		else handler->status = FS_DISCOVERY_NOT_FOUND;
	}
 ...
 *
 */
FsTransactionID FsDelegateRamdisk::requestRead(Thread *requester, FsNode *node, int64_t length, Contextual<uint8_t*> buffer, FileDescriptorContent *fd, FsTransactionHandlerRead *handler)
{
	...
	int64_t copyAmount = ((fd->offset + length) >= ramdiskNode->datalength) ? (ramdiskNode->datalength - fd->offset) : length;
	if (copyAmount > 0)
	{
 ...
 *
 */
void FsDelegateRamdisk::finishRead(Thread *requester, FsReadStatus *outStatus, int64_t *outResult, FileDescriptorContent *fd)
{
}
 ...
 *
 */
FsTransactionID FsDelegateRamdisk::requestWrite(Thread *requester, FsNode *node, int64_t length, Contextual<uint8_t*> buffer, FileDescriptorContent *fd, FsTransactionHandlerWrite *handler)
{
	// start/repeat transaction
	FsTransactionID id;
	if (handler->wantsRepeatTransaction()) id = handler->getRepeatedTransaction();
	...
	if (fd->openFlags & FILE_FLAG_MODE_APPEND) fd->offset = ramdiskNode->datalength;
 ...
 *
 */
void FsDelegateRamdisk::finishWrite(Thread *requester, FsWriteStatus *outStatus, int64_t *outResult, FileDescriptorContent *fd)
{
}
 ...
 *
 */
FsTransactionID FsDelegateRamdisk::requestGetLength(Thread *requester, FsNode *node, FsTransactionHandlerGetLength *handler)
{
	// the ramdisk handler is doing it's work immediately and doesn't request another process
	FsTransactionID id = FsTransactionStore::nextTransaction();
 ...
 *
 */
FsTransactionID FsDelegateRamdisk::requestDirectoryRefresh(Thread *requester, FsNode *folder, FsTransactionHandlerDirectoryRefresh *handler)
{
	FsTransactionID id = FsTransactionStore::nextTransaction();

	RamdiskEntry *rdFolder = EvaKernel::ramdisk->findById(folder->physFsID);
	if (rdFolder == 0) handler->status = FS_DIRECTORY_REFRESH_ERROR;
	...
	else
	{
		...
		while ((rdChild = EvaKernel::ramdisk->getChildAt(folder->physFsID, position++)) != 0)
		{
			...
			// if not, create it
			if (fsChild == 0) fsChild = createVfsNode(rdChild, folder);
		}
 ...
 *
 */
void FsDelegateRamdisk::finishDirectoryRefresh(Thread *requester, FsTransactionHandlerDirectoryRefresh *handler)
{
}
 ...
 *
 */
FsTransactionID FsDelegateRamdisk::requestOpen(Thread *requester, FsNode *node, char *filename, int32_t flags, int32_t mode, FsTransactionHandlerOpen *handler)
{
	FsTransactionID id = FsTransactionStore::nextTransaction();
	...
	if (handler->discoveryStatus == FS_DISCOVERY_SUCCESSFUL)
	{
		...
		{
			logWarn("%! only files can be opened, given node ('%s') was a %i", "filesystem", ramdiskNode->name, ramdiskNode->type);
	...
	else if (handler->discoveryStatus == FS_DISCOVERY_NOT_FOUND)
	{
		...
		// return with failure
		else handler->status = FS_OPEN_NOT_FOUND;
	}
 ...
 *
 */
FsTransactionID FsDelegateRamdisk::requestClose(Thread *requester, FsTransactionHandlerClose *handler, FileDescriptorContent *fd, FsNode *node)
{
	FsTransactionID id = FsTransactionStore::nextTransaction();
