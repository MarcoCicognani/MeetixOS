...
SYSCALL_HANDLER(createEmptyProcess)
{
	...
	if (parentProcess->securityLevel == SECURITY_LEVEL_KERNEL)
	{
		...
		logDebug("%! (%i:%i) created empty process %i", "syscall", parentProcess->main->id, currentThread->id, emptyMainThread->id);
	}

	else
	{
		logWarn("%! (%i:%i) error: insufficient permissions: create empty process", "syscall", parentProcess->main->id, currentThread->id);
	}
...
/**
 * Destroy the cli buffer of the process and returns to the currentThread the previous provided arguments
 */
...
SYSCALL_HANDLER(createPagesInSpace)
{
	...
	if (process->securityLevel == SECURITY_LEVEL_KERNEL)
	{
		...
		if (numberOfPages > 0 && numberOfPages <= CREATE_PAGE_IN_SPACE_MAXIMUM_PAGES)
		{
			...
			if (!targetProcess->imageStart || targetProcess->imageStart > virtualAddressInTargetSpace) targetProcess->imageStart = virtualAddressInTargetSpace;
			if (!targetProcess->imageEnd || targetProcess->imageEnd < (virtualAddressInTargetSpace + numberOfPages * PAGE_SIZE)) targetProcess->imageEnd = virtualAddressInTargetSpace + numberOfPages * PAGE_SIZE;
...
/**
 * Something is gone wrong, the creation process has encountered an error, we have to destroy the task under construction
 */
...
SYSCALL_HANDLER(writeTlsMasterForProcess)
{
	...
	if (process->securityLevel == SECURITY_LEVEL_KERNEL)
	{
		...
		VirtualAddress tlsMasterVirt = targetProcess->virtualRanges.allocate(requiredPages, PROC_VIRTUAL_RANGE_FLAG_PHYSICAL_OWNER);
...
SYSCALL_HANDLER(ramdiskSpawn)
{
	...
	if (process->securityLevel == SECURITY_LEVEL_KERNEL)
	{
		...
		Elf32SpawnStatus status = Elf32Loader::spawnFromRamdisk(data->path, SECURITY_LEVEL_APPLICATION, &spawnedThread, false, THREAD_PRIORITY_NORMAL);
		...
		if (status == Elf32SpawnStatus::SUCCESSFUL)
		{
			...
			logDebug("%! (%i:%i) spawn '%s' from ramdisk -> %i <%h-%h> #%i", "syscall", process->main->id, currentThread->id, data->path, spawnedProcess->imageStart, spawnedProcess->imageEnd, spawnedThread->id, spawnedProcess->securityLevel);
		}
...
SYSCALL_HANDLER(createThread)
{
	...
	if (thread != 0)
	{
		logDebug("%! (%i:%i) created thread %i", "syscall", currentThread->process->main->id, currentThread->id, thread->id);
	...
	{
		logWarn("%! (%i:%i) failed to create thread", "syscall", currentThread->process->main->id, currentThread->id);
		data->threadID = -1;
