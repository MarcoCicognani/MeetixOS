		...
		switch (ip[0])
		{
			...
			case 0x9C:
			{
				if (operands32)
				{
					...
					if (current->getVm86Information()->cpuIf) esp[0] |= EFLAG_IF;
					else esp[0] &= ~EFLAG_IF;
				}
				...
				else
				{
					...
					if (current->getVm86Information()->cpuIf) sp[0] |= EFLAG_IF;
					else sp[0] &= ~EFLAG_IF;
				}
			...
			case 0xCD:
			{
				...
				if (current->getVm86Information()->cpuIf) sp[2] |= EFLAG_IF;
				else sp[2] &= ~EFLAG_IF;
			...
			case 0xEF:
			{
				IOports::writeShort((uint16_t) ctx->defaultFrame.edx, (uint16_t) ctx->defaultFrame.eax);
				++ctx->defaultFrame.eip;
				return VMResult::SUCCESSFUL;
			}
			...
			default:
			{
				logWarn("%! unhandled opcode %h at linear location %h", "vm86", (uint32_t ) ip[0], ip);
