 ...
 * kernel (end of this is delimited by the parameter given to initialize). Identity-maps the memory
 ...
 */
bool PagingInitializer::mapPageToRecursiveDirectory(uint32_t virtualAddress,
                                                    uint32_t physicalAddress,
                                                    uint32_t tableFlags,
                                                    uint32_t pageFlags) {
    // security check
    if ( (virtualAddress & PAGE_ALIGN_MASK) || (physicalAddress & PAGE_ALIGN_MASK) )
        EvaLoader::panic("%! tried to map unaligned addresses: virt %h to phys %h",
                         "paging",
                         virtualAddress,
                         physicalAddress);
    ...
    if ( !table[pi] ) {
        ...
        logDebug(
            "%! tried to map area that was already mapped, virt %h -> phys %h, table contains %h",
            "paging",
            virtualAddress,
            physicalAddress,
            table[pi]);
 ...
 */
void PagingInitializer::relocateMultibootModules(PageDirectory pageDirectory,
                                                 uint32_t      reservedAreaEnd) {
    ...
    for ( uint32_t i = 0; i < mbInfo->modulesCount; i++ ) {
        ...
        MultibootModule* module
            = (MultibootModule*)(mbInfo->modulesAddress + sizeof(MultibootModule) * i);
        ...
        for ( uint32_t off = 0; off < modPhysEndAligned - modPhysStartAligned;
              off          = off + PAGE_SIZE ) {
            mapPage(pageDirectory,
                    modVirtStartAligned + off,
                    modPhysStartAligned + off,
                    DEFAULT_USER_TABLE_FLAGS,
                    DEFAULT_USER_PAGE_FLAGS);
        }

        // Finish relocation by updating multiboot structure
        logDebugn("%! relocated, mapped phys %h-%h",
                  "mmodule",
                  module->moduleStart,
                  module->moduleEnd);
 ...
 * @param pageFlags:		the page flags to apply
 */
void PagingInitializer::identityMap(uint32_t* directory,
                                    uint32_t  start,
                                    uint32_t  end,
                                    uint32_t  tableFlags,
                                    uint32_t  pageFlags) {
 ...
 * @param pageFlags:		the page flags to apply
 */
void PagingInitializer::mapPage(uint32_t* directory,
