...
/**
 * Waiter implementation used for a specific transaction to be finished. Once the transaction is finished,
...
public:
	 ...
	 *							This handler will be deleted after the finish-transaction function was called
	 * @param transactionID:	the id of the transaction that we handle to
	 * @param delegate:			the filesystem delegate
	 */
	WaiterFsTransaction(FsTransactionHandler *handler, FsTransactionID transactionID, FsDelegate *delegate) : handler(handler), transactionID(transactionID), delegate(delegate) {}
	 ...
	 */
	static bool isTransactionWaiting(Thread *task, FsTransactionHandler *handler, FsTransactionID transactionID, FsDelegate *delegate)
	{
		FsTransactionStatus status = FsTransactionStore::getStatus(transactionID);

		// transaction waits for something, same transaction shall be repeated (could not be fulfilled in the first attempt)
		if (status == FS_TRANSACTION_WAITING) return true;
		else if (status == FS_TRANSACTION_REPEAT)
		{
			...
			if (restartStatus != FS_TRANSACTION_START_FAILED) return true;
		...
		else if (status == FS_TRANSACTION_FINISHED)
		{
			...
			if (stat == FS_TRANSACTION_HANDLING_REPEAT_WITH_SAME_HANDLER) return true;
			...
			FsTransactionStore::removeTransaction(transactionID);
			logInfo("%! unknown finish handler status (%i) while waiting for transaction", "filesystem", stat);
			delete handler;
			return false;
		}
		...
		FsTransactionStore::removeTransaction(transactionID);
		logInfo("%! unknown transaction status (%i) while waiting for transaction", "filesystem", status);
		delete handler;
		return false;
	}
