...
void Scheduler::moveToWaitQueue(Thread *thread)
{
	...
	if (moveEntry == currentEntry) currentEntry = 0;
}
	...
	if (milliseconds - lastDebugoutMillis > 10000)
	{
		...
		while (entry)
		{
			...
			logInfo("%# running - %i:%i, eip: %h, waiter: %s, name: %s, rounds: %i", thr->process->main->id, thr->id, thr->cpuState->eip,
					(thr->waitManager == 0 ? "-" : thr->waitManager->debugName()), (thr->getIdentifier() == 0 ? "-" : thr->getIdentifier()),
					thr->rounds);
			entry = entry->next;
		}
		entry = waitQueue;
...
uint32_t Scheduler::count(ThreadType type)
{
	...
	while (entry)
	{
		if (entry->value->type == type) ++count;
		entry = entry->next;
	}
...
uint32_t Scheduler::getTaskIDs(Tid *out, uint32_t len, ThreadType type)
{
	...
	while (pos < len && entry)
	{
		if (entry->value->type == type) out[pos++] = entry->value->id;
		entry = entry->next;
	}
...
void Scheduler::_remove(Thread *thread)
{
	...
	if (!entry) entry = _removeFromQueue(&waitQueue, thread);
...
void Scheduler::_checkWaitingState(Thread *thread)
{
	...
	if (thread->waitManager != 0)
	{
		if (thread->checkWaiting())
		{
			...
			thread->waitCount++;
			if (!(thread->waitCount % 500000)) _printDeadlockWarning();
		}
