...
public:
	 ...
	 */
	IoApic(uint32_t id, PhysicalAddress physicalAddress, uint32_t globalSystemInterruptBase, IoApic *next) :
		id(id), next(next), physicalAddress(physicalAddress), virtualAddress(0), globalSystemInterruptBase(globalSystemInterruptBase), redirectEntryCount(0)
	{}
	 ...
	 */
	inline uint32_t getGlobalSystemInterruptBase() const { return globalSystemInterruptBase; }
	 ...
	 */
	inline uint32_t getRedirectEntryCount() const { return redirectEntryCount; }
	 ...
	 */
	inline IoApic *getNext() const { return next; }
