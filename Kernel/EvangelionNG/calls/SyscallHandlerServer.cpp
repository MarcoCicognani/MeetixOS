    ...
    if ( *data->identifier ) {
        ...
        if ( server ) {
            ...
            if ( status == MESSAGE_SEND_STATUS_SUCCESSFUL ) {
                ...
                data->status = SERVER_MANAGE_STATUS_COMMAND_SEND_FAIL;
        }

        else
            data->status = SERVER_MANAGE_STATUS_SERVER_NOT_FOUND;
    }

    else
        data->status = SERVER_MANAGE_STATUS_INVALID_IDENTIFIER;
...
SYSCALL_HANDLER(registerAsServer) {
    ...
    if ( currentThread->type == THREAD_TYPE_MAIN ) {
        ...
        if ( Tasking::addServer(process) ) {
            // the minimum security level permitted to apply for a server is the
            // SECURITY_LEVEL_SERVER
            // if (process->securityLevel != data->securityLevel)
            //    ThreadManager::applySecurityLevel(process->main->cpuState, data->securityLevel <=
            //    SECURITY_LEVEL_SERVER ? data->securityLevel : SECURITY_LEVEL_SERVER);
            data->status = REGISTER_AS_SERVER_STATUS_SUCCESS;
        }

        else
            data->status = REGISTER_AS_SERVER_STATUS_ALREADY_REGISTERED;
    }

    else
        data->status = REGISTER_AS_SERVER_STATUS_NON_MAIN;
