...
SYSCALL_HANDLER(ramdiskInfo) {
    ...
    if ( entry ) {
        ...
        data->type = RAMDISK_ENTRY_TYPE_UNKNOWN;
...
SYSCALL_HANDLER(ramdiskRead) {
    ...
    if ( entry && entry->type == RAMDISK_ENTRY_TYPE_FILE ) {
        ...
        int byteCount
            = (((long)bytesFromOffset) > (long)data->length) ? data->length : bytesFromOffset;
        if ( byteCount > 0 ) {
...
SYSCALL_HANDLER(ramdiskChildCount) {
    SyscallRamdiskChildCount* data
        = (SyscallRamdiskChildCount*)SYSCALL_DATA(currentThread->cpuState);
...
/**
 * Returns the id of the child of the parent node (with the given id) at the given index on the
 * ramdisk.
 */
