...
public:
	 ...
	 */
	FsTransactionHandlerDiscoverySetCwd(char *absolutePathIn, Contextual<SyscallFsSetWorkingDirectory*> data, Thread *unspawnedTarget) : FsTransactionHandlerDiscovery(absolutePathIn), data(data), unspawnedTarget(unspawnedTarget) {}
	...
	virtual FsTransactionHandlerFinishStatus afterFinishTransaction(Thread *task)
	{
		if (status == FS_DISCOVERY_SUCCESSFUL)
		{
			if (!(node->type == FS_NODE_TYPE_PIPE || node->type == FS_NODE_TYPE_FILE))
			{
				...
				logDebug("%! cwd of process %i is now '%s'", "filesystem", target->process->main->id, target->process->workingDirectory);
			}

			else
			{
				data()->result = SET_WORKING_DIRECTORY_NOT_A_FOLDER;
				logInfo("%! could not set current working directory to '%s', not a folder", "filesystem", data()->path);
			}
		...
		else if (status == FS_DISCOVERY_NOT_FOUND)
		{
			...
			logInfo("%! could not set current working directory to '%s', node not found", "filesystem", data()->path);
		}

		else if (status == FS_DISCOVERY_ERROR)
		{
			data()->result = SET_WORKING_DIRECTORY_ERROR;
			logInfo("%! could not set current working directory to '%s', missing delegate on way", "filesystem", data()->path);
		}

		else if (status == FS_DISCOVERY_BUSY)
		{
			data()->result = SET_WORKING_DIRECTORY_ERROR;
			logInfo("%! could not set current working directory to '%s', delegate busy", "filesystem", data()->path);
		}
