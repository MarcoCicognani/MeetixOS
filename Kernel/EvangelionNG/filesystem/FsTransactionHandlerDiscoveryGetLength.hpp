...
public:
     ...
     *
     */
    FsTransactionHandlerDiscoveryGetLength(char*                        absolutePathIn,
                                           bool                         followSymlinks,
                                           Contextual<SyscallFsLength*> data)
        : FsTransactionHandlerDiscovery(absolutePathIn, followSymlinks), data(data) {
    }
    ...
    virtual FsTransactionHandlerFinishStatus afterFinishTransaction(Thread* thread) {
        if ( status == FS_DISCOVERY_SUCCESSFUL ) {
            ...
            FsTransactionHandlerGetLengthDefault* handler
                = new FsTransactionHandlerGetLengthDefault(data, node);
