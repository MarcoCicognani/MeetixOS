...
public:
	 ...
	 *
	 */
	FsTransactionHandlerGetLengthSeek(FileDescriptorContent *fd, FsNode *node, Contextual<SyscallFsSeek*> data) : FsTransactionHandlerGetLength(node), fd(fd), data(data) {}
	...
	virtual void performAfterwork(Thread *thread)
	{
		if (status == FS_LENGTH_SUCCESSFUL)
		{
			...
			if (data()->mode == FS_SEEK_CUR) fd->offset += data()->amount;
			else if (data()->mode == FS_SEEK_SET) fd->offset = data()->amount;
			else if (data()->mode == FS_SEEK_END) fd->offset = length - data()->amount;

			// validate offset
			if (fd->offset > length) fd->offset = length;
