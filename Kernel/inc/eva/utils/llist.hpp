 ...
 */
template<class Type_t> class llist
{
public:
	/**
	 * node descriptor
	...
	public:
		 ...
		 */
		iterator(llist *list, bool reverse, Node_t *first = 0, uint32_t firstIndex = 0) : list(list), _current(0), _index(firstIndex)
		{
			if (first) _current = first;
			else if (!reverse && list->_firstNode) _current = list->_firstNode;
		 ...
		 */
		Node_t *operator->() const { return _current; }
		 ...
		 */
		Type_t operator*() const { return (_current->value); }
		...
		friend bool operator!(const iterator &it) { return ((it.list == 0) || (it._current == 0)); }
	...
	private:
		...
		iterator &increment(uint32_t size)
		{
			// increment of size
			uint32_t i = 0;
			while ((_current != 0) && (_current->next != 0) && (_index < list->_size) && (i <= size))
			{
				_current = _current->next;
				_index++;
				i++;
	...
	Type_t get(uint32_t position)
	{
		...
		else if (position < _size)
		{
			...
			while (_start < _end)
			{
				...
				if (!reverse && current && current->next) current = current->next;
				else if (current && current->prev) current = current->prev;
				_start++;
			}
	 ...
	 * @return the value of the node requested if exist
	 */
	inline Type_t operator[](uint32_t position) { return get(position); }
	...
	iterator find(Type_t value)
	{
		...
		if (_size >= 2)
		{
			...
			if (found) return iterator(this, false, current, index);
		}
	...
	inline Type_t front()
	{
		...
		if (_firstNode) return _firstNode->value;
	 ...
	 */
	inline void popFront() { remove(_firstNode); }
	...
	inline Type_t back()
	{
		...
		if (_lastNode) return _lastNode->value;
	 ...
	 */
	inline void popBack() { remove(_lastNode); }
	 ...
	 */
	inline void erase(const iterator &it) { remove(it._current); }
	 ...
	 */
	inline void erase(Type_t value) { remove(find(value)._current); }
	...
	void sort(bool (*compare)(const Type_t &first, const Type_t &second))
	{
		for (Node_t *first = _firstNode; first != 0; first = first->next)
			for (Node_t *second = first->next; second != 0; second = second->next)
				if (compare(first->value, second->value)) swap(first, second);
	}
