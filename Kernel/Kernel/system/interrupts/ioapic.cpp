...
void IoApic::createMapping() {
    ...
    if ( !virtualAddress )
        EvaKernel::panic("%! could not get a virtual range for mapping", "ioapic");
    ...
    AddressSpace::map(PAGE_ALIGN_DOWN(virtualAddress),
                      PAGE_ALIGN_DOWN(physicalAddress),
                      DEFAULT_KERNEL_TABLE_FLAGS,
                      DEFAULT_KERNEL_PAGE_FLAGS);
    AddressSpace::map(PAGE_ALIGN_DOWN(virtualAddress) + PAGE_SIZE,
                      PAGE_ALIGN_DOWN(physicalAddress) + PAGE_SIZE,
                      DEFAULT_KERNEL_TABLE_FLAGS,
                      DEFAULT_KERNEL_PAGE_FLAGS);
}
...
void IoApic::initialize() {
    ...
    if ( reportedId != id ) {
        logWarn("%! has different ID (%i) than what ACPI reported (%i), reprogramming",
                "ioapic",
                idValue,
                reportedId);
    ...
    logInfo("%! id %i: version %i, redirect entries: %i",
            "ioapic",
            id,
            version,
            redirectEntryCount);
