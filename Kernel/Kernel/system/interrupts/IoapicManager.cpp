 ...
 */
bool IoApicManager::createIsaRedirectionEntry(uint32_t source,
                                              uint32_t irq,
                                              uint32_t destinationApic) {
    ...
    if ( n ) {
        ...
        redirectionTableEntry
            |= IOAPIC_REDTBL_DESTINATION_MAKE(destinationApic,
                                              IOAPIC_REDTBL_DESTINATION_FLAG_PHYSICAL);
...
IoApic* IoApicManager::getResponsibleFor(uint32_t source) {
    IoApic* n = first;
    while ( n ) {
        if ( source >= n->getGlobalSystemInterruptBase()
             && source < (n->getGlobalSystemInterruptBase() + n->getRedirectEntryCount()) )
            break;
        n = n->getNext();
    }
    return n;
