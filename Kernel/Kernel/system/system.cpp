...
void System::initializeBasicSystemPackage(PhysAddr initialPageDirectoryPhysical) {
    ...
    if ( Processor::supportsCpuid() ) {
        ...
        EvaKernel::panic("%! no CPUID support", "cpu");
    }
    ...
    if ( Processor::hasFeature(CpuidStandardEdxFeature::APIC) ) {
        logDebug("%! APIC available", "cpu");
    } else
        EvaKernel::panic("%! no APIC available", "cpu");
    ...
    if ( Acpi::hasEntries() ) {
        ...
        if ( cur )
            Madt::parse(cur->header);
    }

    else
        EvaKernel::panic("%! ACPI info not available", "system");
    ...
    if ( Lapic::isPrepared() && IoApicManager::areAvailable() && processorList ) {
        ...
        logInfo("%! %i available core%s",
                "system",
                processorsAvailable,
                (processorsAvailable > 1 ? "s" : ""));
        ...
        EvaKernel::panic("%! pic compatibility mode not implemented. apic/ioapic required!",
                         "system");
}
