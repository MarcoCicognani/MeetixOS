 ...
 * @param securityLevel:			the security level to be assigned to the created
 * process
 ...
 */
Elf32SpawnStatus Elf32Loader::spawnFromRamdisk(const char*    path,
    ...
    if ( !entry || entry->type != RAMDISK_ENTRY_TYPE_FILE )
        return Elf32SpawnStatus::FILE_NOT_FOUND;
...
Elf32ValidationStatus Elf32Loader::validate(Elf32Ehdr* header) {
    ...
    if ( header->e_type != ET_EXEC )
        return Elf32ValidationStatus::NOT_EXECUTABLE;

    // Must be i386 architecture compatible
    if ( header->e_machine != EM_386 )
        return Elf32ValidationStatus::NOT_I386;

    // Must be 32 bit
    if ( header->e_ident[EI_CLASS] != ELFCLASS32 )
        return Elf32ValidationStatus::NOT_32BIT;

    // Must be little endian
    if ( header->e_ident[EI_DATA] != ELFDATA2LSB )
        return Elf32ValidationStatus::NOT_LITTLE_ENDIAN;

    // Must comply to current ELF standard
    if ( header->e_version != EV_CURRENT )
        return Elf32ValidationStatus::NOT_STANDARD_ELF;
 ...
 * the start and end address of the executable image in memory are written to the respective out
 * parameters.
 */
