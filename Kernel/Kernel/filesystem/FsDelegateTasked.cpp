 ...
 *
 */
FsDelegateTasked::FsDelegateTasked(Thread* delegateThread)
    : transactionStoragePhys(0), delegateThread(delegateThread) {
}
...
bool FsDelegateTasked::prepare(VirtAddr* outTransactionStorage) {
    ...
    if ( transactionStorageAddress == 0 ) {
        logWarn("%! failed to allocate virtual range for transaction storage when trying to create "
                "tasked delegate",
                "filesystem");
        return false;
    }
    ...
    if ( transactionStoragePhys == 0 ) {
        logWarn("%! failed to allocate physical page for transaction storage when trying to create "
                "tasked delegate",
                "filesystem");
        return false;
    }
    ...
    AddressSpace::map(transactionStorageAddress,
                      transactionStoragePhys,
                      DEFAULT_USER_TABLE_FLAGS,
                      DEFAULT_USER_PAGE_FLAGS);
    AddressSpace::switchToSpace(current);
    logDebug("%! fs delegate transaction storage created at %h of process %i",
             "filesystem",
             transactionStorageAddress,
             delegateThread->id);

    *outTransactionStorage = transactionStorageAddress;
    transactionStorage.set((void*)transactionStorageAddress,
                           delegateThread->process->pageDirectory);
    return true;
}
 ...
 *
 */
FsTransactionID FsDelegateTasked::requestDiscovery(Thread*                        requester,
                                                   FsNode*                        parent,
                                                   char*                          child,
    ...
    if ( configurationFine == false )
        ...
        if ( sendStatus == MESSAGE_SEND_STATUS_SUCCESSFUL )
            ...
            logWarn("%! message queue was full when trying to request discovery to fs delegate",
                    "filesystem");
            ...
            logWarn("%! message sending failed when trying to request discovery to fs delegate",
                    "filesystem");
            handler->status = FS_DISCOVERY_ERROR;
            FsTransactionStore::setStatus(id, FS_TRANSACTION_FINISHED);
        }
...
void FsDelegateTasked::finishDiscovery(Thread* requester, FsTransactionHandlerDiscovery* handler) {
    ...
    AddressSpace::switchToSpace(delegateThread->process->pageDirectory);
    FsTaskedDelegateTransactionStorageDiscovery* dspace
        = (FsTaskedDelegateTransactionStorageDiscovery*)transactionStorage();
 ...
 *
 */
FsTransactionID FsDelegateTasked::requestRead(Thread*                   requester,
                                              FsNode*                   node,
                                              int64_t                   length,
    ...
    FsTaskedDelegateTransactionStorageRead* disc
        = (FsTaskedDelegateTransactionStorageRead*)transactionStorage();
    ...
    if ( sendStatus == MESSAGE_SEND_STATUS_SUCCESSFUL ) {
        ...
        logWarn("%! message queue was full when trying to request read to fs delegate",
                "filesystem");
        ...
        logWarn("%! message sending failed when trying to request read to fs delegate",
                "filesystem");
 ...
 *
 */
void FsDelegateTasked::finishRead(Thread*                requester,
    ...
    FsTaskedDelegateTransactionStorageRead* rspace
        = (FsTaskedDelegateTransactionStorageRead*)transactionStorage();
...
FsTransactionID FsDelegateTasked::requestWrite(Thread*                    requester,
    ...
    FsTaskedDelegateTransactionStorageWrite* disc
    ...
    if ( sendStatus == MESSAGE_SEND_STATUS_SUCCESSFUL )
        ...
        logWarn("%! message queue was full when trying to request write to fs delegate",
                "filesystem");
        ...
        logWarn("%! message sending failed when trying to request write to fs delegate",
                "filesystem");
 ...
 *
 */
void FsDelegateTasked::finishWrite(Thread*                requester,
 ...
 *
 */
FsTransactionID FsDelegateTasked::requestGetLength(Thread*                        requester,
                                                   FsNode*                        node,
    ...
    FsTaskedDelegateTransactionStorageGetLength* disc
    ...
    if ( sendStatus == MESSAGE_SEND_STATUS_SUCCESSFUL )
        ...
        logWarn("%! message queue was full when trying to request get-length to fs delegate",
                "filesystem");
        ...
        logWarn("%! message sending failed when trying to request get-length to fs delegate",
                "filesystem");
 ...
 */
FsTransactionID
FsDelegateTasked::requestDirectoryRefresh(Thread*                               requester,
                                          FsNode*                               node,
    ...
    FsTaskedDelegateTransactionStorageDirectoryRefresh* disc
        = (FsTaskedDelegateTransactionStorageDirectoryRefresh*)transactionStorage();
    ...
    if ( sendStatus == MESSAGE_SEND_STATUS_SUCCESSFUL )
        ...
        logWarn("%! message queue was full when trying to request read-directory on fs delegate",
                "filesystem");
        ...
        logWarn("%! message sending failed when trying to request read-directory on fs delegate",
                "filesystem");
 ...
 *
 */
void FsDelegateTasked::finishDirectoryRefresh(Thread*                               requester,
                                              FsTransactionHandlerDirectoryRefresh* handler) {
    // save current directory
    PageDirectory current = AddressSpace::getCurrentSpace();
    ...
    FsTaskedDelegateTransactionStorageDirectoryRefresh* storage
        = (FsTaskedDelegateTransactionStorageDirectoryRefresh*)transactionStorage();
...
FsTransactionID FsDelegateTasked::requestOpen(Thread*                   requester,
    ...
    FsTaskedDelegateTransactionStorageOpen* disc
    ...
    if ( childlen > FILENAME_MAX ) {
        ...
        Memory::copy(disc->name, filename, childlen + 1);
    ...
    if ( sendStatus == MESSAGE_SEND_STATUS_SUCCESSFUL )
        ...
        logWarn("%! message queue was full when trying to request open to fs delegate",
                "filesystem");
        ...
        logWarn("%! message sending failed when trying to request open to fs delegate",
                "filesystem");
        handler->status = FS_OPEN_ERROR;
        FsTransactionStore::setStatus(id, FS_TRANSACTION_FINISHED);
    }
...
void FsDelegateTasked::finishOpen(Thread* requester, FsTransactionHandlerOpen* handler) {
    ...
    FsTaskedDelegateTransactionStorageOpen* storage
        = (FsTaskedDelegateTransactionStorageOpen*)transactionStorage();
 ...
 *
 */
FsTransactionID FsDelegateTasked::requestClose(Thread*                    requester,
    ...
    FsTaskedDelegateTransactionStorageClose* disc
    ...
    if ( sendStatus == MESSAGE_SEND_STATUS_SUCCESSFUL )
        ...
        logWarn("%! message queue was full when trying to request close on fs delegate",
                "filesystem");
        ...
        logWarn("%! message sending failed when trying to request close on fs delegate",
                "filesystem");
