 ...
 *
 */
void FileSystem::findExisting(char*    absolutePath,
                              FsNode** outParent,
                              FsNode** outChild,
                              char*    nameCurrent,
                              bool     followSymlinks) {
...
void FileSystem::processForked(Pid source, Pid fork) {
    ...
    // clone each entry
    for ( auto iter = sourceTable->descriptors.begin(); iter != sourceTable->descriptors.end();
          ++iter ) {
        FileDescriptorContent* content = iter->value;
...
void FileSystem::getRealPathToNode(FsNode* node, char* out) {
    ...
    while ( current != 0 ) {
        // on root we can stop
        if ( current->type == FS_NODE_TYPE_ROOT )
            break;
...
FsRegisterAsDelegateStatus FileSystem::createDelegate(Thread*     thread,
    ...
    *outMountpointID = mountpoint->id;
    ...
    logInfo("%! mountpoint '%s' (node id %i) is handled by delegate task %i",
            "filesystem",
            mountpoint->name,
            mountpoint->id,
            thread->id);
...
FileHandle FileSystem::clonefd(FileHandle       sourceFd,
    ...
    if ( targetFd != -1 ) {
        ...
        if ( targetNode )
            unmapFile(sourcePid, targetNode, targetFdContent);
    }
