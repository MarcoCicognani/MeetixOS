...
public:
	 ...
	 */
	Rectangle(int32_t x, int32_t y, int32_t width, int32_t height) : x(x), y(y), width(width), height(height) {}
	...
	void setTop(int _top) { y = _top; }

	/**
	 * manage X
	 */
	int getLeft() const { return x; }
	void setLeft(int _left) { x = _left; }

	/**
	 * manage height
	 */
	int getBottom() const { return y + height; }
	void setBottom(int32_t _bottom) { height = _bottom - y; }

	/**
	 * manage width
	 */
	int getRight() const { return x + width; }
	void setRight(int _right) { width = _right - x; }

	/**
	 * set x and y from p
	 */
	void setStart(const Point &p) { x = p.x; y = p.y; }

	/**
	 * return a point with x and y coordinates
	 */
	Point getStart() const { return Point(x, y); }

	/**
	 * set the end of rectangle
	 */
	void setEnd(const Point &p) { width = p.x - x; height = p.y - y; }

	/**
	 * return the end of rectangle
	 */
	Point getEnd() const { return Point(x + width, y + height); }
