 ...
 *
 * @param flagp: 	pointer to a fexcept_t object with a representation of floating-point exceptions
 * @param excepts:	bitmask value
 ...
 * The value returned is the bitwise OR representation of the subset of excepts that are currently set in the floating point environment.
 * Or zero, if none of the exceptions in excepts are currently set.
 ...
 *
 * @param envp:		    Pointer to a fenv_t object where the state of the floating-point environment is stored.
 ...
 * The floating point environment is a set of status flags and control modes affecting floating-point calculations
 * (including both floating-point exceptions and the rounding direction mode). If successful, the function changes the current state of the
 * floating-point environment without actually raising the exceptions specified in such state.
 *
...
/**
 * Attempts to establish the state of the floating-point environment as represented by the object pointed by envp.
 * It then attempts to raise the exceptions that were set in the floating-point environment before the call.
 *
 * @param envp:		Either a pointer to a fenv_t value
