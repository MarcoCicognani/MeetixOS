 ...
 */
void TextLayouter::layout(cairo_t*             cr,
                          const char*          text,
                          Font_t*              font,
                          int32_t              size,
                          Rectangle            bounds,
    ...
    // free old buffer
    if ( previousGlyphBuffer != nullptr && layout->glyphBuffer != previousGlyphBuffer )
        free(previousGlyphBuffer);
    if ( previousClusterBuffer != nullptr && layout->clusterBuffer != previousClusterBuffer )
        free(previousClusterBuffer);
    ...
    if ( stat == CAIRO_STATUS_SUCCESS ) {
        ...
        for ( int i = 0; i < layout->clusterCount; i++ ) {
            ...
            cairo_scaled_font_glyph_extents(scaledFace,
                                            positioned.glyph,
                                            positioned.glyphCount,
                                            &extents);
