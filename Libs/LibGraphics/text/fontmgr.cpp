...
void FontManager::initializeEngine() {
    ...
    if ( error )
        Utils::log("freetype2 failed at FT_Init_FreeType with error code %i", error);
}
...
void FontManager::destroyEngine() {
    ...
    if ( error )
        Utils::log("freetype2 failed at FT_Done_Library with error code %i", error);
}
 ...
 */
bool FontManager::createFont(const std::string& name,
                             uint8_t*           source,
