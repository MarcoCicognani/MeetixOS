...
#undef weak_alias
#define weak_alias(old_alias, new_alias) extern __typeof(old_alias) new_alias __attribute__((weak, alias(#old_alias)))
...
static __inline unsigned __FLOAT_BITS(float f) {
    union {
        float    m_f;
        unsigned m_i;
    } fu;
...
static __inline unsigned long long __DOUBLE_BITS(double d) {
    union {
        double             m_d;
