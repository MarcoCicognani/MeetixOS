...
static int hasarg(char c, const char *optstring)
{
	for(size_t i = 0; optstring[i]; ++i)
		if (optstring[i] == c) return optstring[i + 1] == ':' ? required_argument : no_argument;
...
int getoptishelp(int argc, char **argv)
{
	...
	for (int arg = 0; arg < argc; ++arg)
		if (!strcmp(argv[1], "-h") || !strcmp(argv[1], "--help") || !strcmp(argv[1], "-?")) return true;
 ...
 */
int getoptlong(int argc, const char *argv[], const char *optstring, struct option *longopts, int *longindex)
{
	static size_t nextchar = 0;
	optarg = NULL;

	if (longindex) optind = *longindex;
	...
	while (optind < argc)
    {
		...
		if (nextchar == 0)
        {
			...
			if (optel[0] != '-' || optel[1] == '\0' || (optel[1] == '-' && optel[2] == '\0')) return -1;
			nextchar = 1;
		}
		...
		if (arg == -1)
        {
			...
			if (optel[1] != '-')
            {
				if (opterr) fprintf(stderr, "Unrecognized option '%c' in argv[%d]=%s\n", val, optind, optel);
				optopt = val;
			}
			else if (opterr) fprintf(stderr, "Unrecognized argument argv[%d]=%s\n", optind, optel);
			val = '?';
		}
		...
		if (arg == required_argument)
        {
            ...
            {
				if (opterr) fprintf(stderr, "Missing argument to option argv[%d]=%s\n", optind, optel);
				return '?';
			}
			else
            {
