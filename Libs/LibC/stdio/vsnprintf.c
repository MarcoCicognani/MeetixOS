...
static isize vcbprintf_vsnprintf_callback(void* param, const char* str, usize len) {
    vcbprintf_vsnprintf_callback_data_t* data = (vcbprintf_vsnprintf_callback_data_t*)param;

    if ( data->m_produced < data->m_out_buffer_len ) {
        usize space    = data->m_out_buffer_len - data->m_produced;
        usize writable = (len < space) ? len : space;
...
int vsnprintf(char* s, usize n, const char* format, va_list arg) {
