...
auto Formatter<ErrnoCode>::new_from_format_applier(FormatApplier format_applier) -> Formatter<ErrnoCode> {
    return Formatter<ErrnoCode>{ Cxx::move(format_applier) };
}

auto Formatter<ErrnoCode>::new_from_parser_result(StringBuilder& string_builder, FormatParser::Result result) -> Formatter<ErrnoCode> {
    return new_from_format_applier(FormatApplier::new_from_parser_result(string_builder, Cxx::move(result)));
}
