...
public:
     ...
     */
    TimeSpan(uint64_t d, uint64_t h, uint64_t m, uint64_t s, uint64_t ms)
     ...
     * operators
     */
    bool operator==(const TimeSpan& tm) const {
        return day == tm.day && hour == tm.day && minute == tm.minute && second == tm.second
     ...
     */
    uint64_t getTotTime() const {
        return (millisecond + (second * SECOND) + (minute * MINUTE) + (hour * HOUR) + (day * DAY));
    }
