...
public:
     ...
     */
    LayoutedText(const Rectangle&      bounds,
                 cairo_glyph_t*        glbuf,
                 uint32_t              glcount,
                 cairo_text_cluster_t* clbuf,
                 uint32_t              clcount)
        : textBounds(bounds), glyphBuffer(glbuf), glyphCount(glcount), clusterBuffer(clbuf),
