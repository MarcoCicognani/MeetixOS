 ...
 */
ButtonMenu::ButtonMenu(const map<string, string> &configuration, const Rectangle &bounds, Geoshape *where)
{
 ...
 */
ButtonMenu::ButtonMenu(const map<string, string> &configuration, const Rectangle &bounds, Window *where)
{
	create(configuration, bounds);
	show(where);
}
...
string ButtonMenu::parseCurrent(const pair<string, string> &current)
{
	...
	else command = current.second;

	// add button to list
	buttons.add(current.first, [command](){ Spawn(command.c_str(), "", "/", SECURITY_LEVEL_APPLICATION); });
...
{
	// configure bounds
	if (index == 0) buttonBounds = Rectangle(0, height, buttonBounds.width, buttonHeight);
	...
	buttons.configure(current.first, buttonBounds, current.first, pngPaths[index], Point(0, 0), ARGB(10, 0, 0, 0), RGB(255, 255, 255));
}
