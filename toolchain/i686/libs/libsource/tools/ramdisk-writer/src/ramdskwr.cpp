...
int main(int argc, char *argv[])
{
	...
	if (argc == 2)
	{
		if (strcmp(argv[1], "--help") == 0)
		{
			...
			std::cout << "  MeetiX OS Ramdisk Writer, by Max Schluessel" << std::endl << std::endl;

			std::cout << "DESCRIPTION" << std::endl;
			std::cout << "  This program generates a ramdisk image from a given source folder." << std::endl;
			std::cout << "  To do so, use the following command syntax:" << std::endl << std::endl;
...
void RamdiskWriter::create(const std::string &sourcePath, const std::string &targetPath)
{
	...
	if (rdignore.is_open())
	{
		...
		while (getline(rdignore, line))
			if (line.length() > 0) ignores.push_back(trim(line));
	}
	...
	try
	{
		...
		if (out.good())
		{
			...
			std::cout << "status: packing folder \"" << sourcePath << "\" to file \"" << targetPath << "\":" << std::endl;

			int64_t pos = out.tellp();
			writeRecursive(sourcePath, sourcePath, "", 0, 0, false);
			int64_t written = out.tellp() - pos;

			std::cout << "status: \"" << targetPath << "\" successfully created, wrote " << written << " bytes" << std::endl;
		}

		// an error was occoured
		else std::cerr << "error: could not write to file '" << targetPath << "'" << std::endl;

	}

	// and exception was occoured
	catch (std::exception &e)
	{
		std::cerr << "error: an error occured while creating the ramdisk: " << e.what() << std::endl;
	}
 ...
 */
void RamdiskWriter::writeRecursive(const std::string &basePath, const std::string &path, const std::string &name, uint32_t contentLength, uint32_t parentId, bool isFile)
{
	...
	else
	{
		...
		else std::cerr << "error: could not open directory: '" << path << "'";
	}
