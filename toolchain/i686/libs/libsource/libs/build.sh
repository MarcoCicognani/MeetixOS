...
if [[ $# -eq 2 ]]; then
    ...
    INC_KERNEL="/home/marco/Scrivania/MeetiX-OS/source/kernel/inc"
fi
...
function getLibType() {
    if [[ $1 ]]; then
        if ([[ $1 == "static" ]] || [[ $1 == "stat" ]] || [[ $1 == "s" ]]); then
            echo "static"
        elif ([[ $1 == "dynamic" ]] || [[ $1 == "dyn" ]] || [[ $1 == "d" ]]); then
            echo "dynamic"
        else
            echo "$MEETIX_LINK_STD"
        fi
    else
        echo "$MEETIX_LINK_STD"
    fi
...
function installHeader() {
    ...
    for dir in $INC/*; do
        ...
        if ([ -d $dir ] && [[ $dir != $INC_C ]] && [[ $dir != $INC_API ]] && [[ $dir != $INC_KERNEL ]]); then
...
function compile() {
    ...
    # assemble sources
    for file in $(find "$SOURCES" -iname "*.s"); do
        changes -c $file
        changed=$?
        if ([ $headersHaveChanged -eq 1 ] || [ $changed -eq 1 ]); then
            out=$(sourceToObject $file)
...
if [[ $target == "all" ]]; then
    ...
    buildLibUtils $2
elif [[ $target == "header" ]]; then
    installHeader $2
elif [[ $target == "driver" ]]; then
    buildLibDriver $2
elif [[ $target == "graphic" ]]; then
    buildLibGraphics $2
elif [[ $target == "tasking" ]]; then
    buildLibTasking $2
