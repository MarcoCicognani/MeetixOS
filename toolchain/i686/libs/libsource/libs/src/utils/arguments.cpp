...
llist<std::string> Arguments::getNonFiltered(int argc, const char *argv[])
{
    ...
    for (int arg = 0; arg < argc; arg++)
        if (argv[arg][0] != '-') nonfiltered.add(std::string(argv[arg]));
...
llist<std::string> Arguments::getFiltered(int argc, const char *argv[])
{
    ...
    // parse all arguments
    for (int arg = 0; arg < argc; arg++)
        if (argv[arg][0] == '-') filtered.add(std::string(argv[arg]));
 ...
 */
template<typename T> void Arguments::split(const std::string &str, char delim, T result)
{
