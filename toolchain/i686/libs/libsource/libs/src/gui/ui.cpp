...
UiOpenStatus UI::open()
{
	...
	if (UiInitialized) return UI_OPEN_STATUS_EXISTING;

	// get window managers id
	Tid windowMgr = TaskGetID(UI_REGISTRATION_THREAD_IDENTIFIER);
	if (windowMgr == -1)
	{
		Utils::log("failed to retrieve task id of window server with identifier '%s'", (char*)UI_REGISTRATION_THREAD_IDENTIFIER);
	...
	if (ReceiveMessageT(responseBuffer(), responseBufferSize, initTx) != MESSAGE_RECEIVE_STATUS_SUCCESSFUL)
	{
...
UiCloseStatus UI::close()
{
	...
	if (!UiInitialized) return UI_CLOSE_ALREADY_CLOSED;
...
{
	if (!UiInitialized) return Dimension();
...
bool UI::setBackground(std::string path)
{
	...
	if (ReceiveMessageT(buffer, bufferSize, tx) == MESSAGE_RECEIVE_STATUS_SUCCESSFUL)
	{
		...
		if (response->status == UI_PROTOCOL_SUCCESS) return true;
	}
...
void UI::eventDispatchThread()
{
	...
	while (true)
	{
		...
		if (stat == MESSAGE_RECEIVE_STATUS_SUCCESSFUL)
		{
			...
			// event message
			UiComponentEventHeader *eventHeader = (UiComponentEventHeader*) MESSAGE_CONTENT(buffer());
		...
		else Utils::log("something went wrong when receiving an event, status code: %i", stat);
	}
}
