...
{
	if (!UiInitialized) return Rectangle();
	...
	if (ReceiveMessageT(buffer(), bufferSize, tx) == MESSAGE_RECEIVE_STATUS_SUCCESSFUL)
	{
		...
		if (response->status == UI_PROTOCOL_SUCCESS) return response->bounds;
	}
...
bool Component::setNumericProperty(int property, uint32_t value)
{
	...
	if (ReceiveMessageT(buffer(), bufferSize, tx) == MESSAGE_RECEIVE_STATUS_SUCCESSFUL)
	{
		UiComponentSetNumericPropertyResponse *response = (UiComponentSetNumericPropertyResponse*) MESSAGE_CONTENT(buffer());
...
bool Component::getNumericProperty(int property, uint32_t* out)
{
	...
	if (ReceiveMessageT(buffer(), bufferSize, tx) == MESSAGE_RECEIVE_STATUS_SUCCESSFUL)
	{
		UiComponentGetNumericPropertyResponse *response = (UiComponentGetNumericPropertyResponse*) MESSAGE_CONTENT(buffer());
...
bool Component::setListener(UiComponentEventType eventType, Listener *newListener)
{
	...
	// check
	if (newListener == nullptr) return false;
...
void Component::handle(UiComponentEventHeader *header)
{
	...
	if (listener != nullptr) listener->process(header);
}
