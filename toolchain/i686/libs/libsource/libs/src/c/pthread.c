...
int pthread_attr_init(pthread_attr_t *attr)
{
    ...
    if (!attr) attr = malloc(sizeof(pthread_attr_t));
...
{
    if (!attr || (detachstate >= 0 && detachstate <= 1)) return EINVAL;
...
int pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine)(void*), void *arg)
{
    ...
    if (threadarguments)
    {
        ...
        if (attr) thread->attr = *attr;
        else thread->attr = PTHREAD_CREATE_JOINABLE;

        // check return status
        if (outStatus != CREATE_THREAD_STATUS_SUCCESSFUL) return -1;
        return 0;
    }
...
{
    if (!thread) return EINVAL;
    if (thread->attr != PTHREAD_CREATE_JOINABLE) return -1;
    
    Join(thread->id);
    if (*retval) *retval = thread->ret_val;
...
{
    if (!thread) return EINVAL;
    if (Kill(thread->id) != KILL_STATUS_SUCCESSFUL) return -1;
    return 0;
}
