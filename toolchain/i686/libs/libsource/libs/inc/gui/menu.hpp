...
public:
	...
	ButtonMenu(const std::map<std::string, std::string> &configuration, const Rectangle &bounds, Geoshape *where);

	// constructor with map of configuration, bounds and window base espected
	ButtonMenu(const std::map<std::string, std::string> &configuration, const Rectangle &bounds, Window *where);
