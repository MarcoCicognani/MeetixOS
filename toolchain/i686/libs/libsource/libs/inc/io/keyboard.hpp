...
public:
	...
	public:
		...
		InfoBasic(const InfoBasic &bcp) :
			pressed(bcp.pressed), ctrl(bcp.ctrl), alt(bcp.alt), shift(bcp.shift), scancode(bcp.scancode) {}
	...
	public:
		 ...
		 * @param scancode:		value to assign to scancode flag
		 */
		Info(const std::string &key, bool pressed, bool ctrl, bool alt, bool shift, uint8_t scancode) :
		 ...
		 * copy constructor
		 */
		Info(const InfoBasic &bcp) :
			key("KEY_NONE"), InfoBasic(bcp) {}
		...
		bool operator<(const Info &other) const
		{
			...
			if (pressed < other.pressed) return true;
			if (ctrl > other.ctrl) return false;
			if (ctrl < other.ctrl) return true;
			if (alt > other.alt) return false;
			if (alt < other.alt) return true;
			if (shift > other.shift) return false;
			if (shift < other.shift) return true;
