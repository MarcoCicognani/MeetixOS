 ...
 * @param-opt outStdio:     is filled with stdio file descriptors, 0 is write end of stdin, 1 is read end of stdout, 2 is read end of stderr
 * @param-opt inStdio:      if supplied, the given descriptors which are valid for the executing process
...
SpawnStatus SpawnPO(const char *path, const char *args, const char *workdir, SecurityLevel securityLevel, Pid *pid, File_t outStdio[3]);
SpawnStatus SpawnPOI(const char *path, const char *args, const char *workdir, SecurityLevel securityLevel, Pid *pid, File_t outStdio[3], File_t inStdio[3]);

/**
 * Register the current process as a kernel 
 * server and return one of the {RegisterAsServerStatus} codes.
 * Update the security level of the process and permits to increase it since the {SECURITY_LEVEL_SERVER}
 *
 ...
 * if it exists, then wait with a receive message the response of the server, that provides data by the <data> parameter
 *
 * @param serverIdentifier:			the name of the server
 * @param command:					a command to send to the server
 * @param data:						a pointer the data that process the server called
 * @param length:					the length of the data buffer
 * @return one of the {ServerManageStatus} codes.
 */
ServerManageStatus ServerManage(const char *serverIdentifier, ServerManageCommand command, void *data, uint32_t length);
 ...
 * @param-opt name:         the name for the thread, the name is appended to the default name created by the kernel
 ...
 * @param-opt mode:    determines how the function blocks when given, default is {MESSAGE_SEND_MODE_BLOCKING}
 ...
 * @param-opt mode:             determines how the function blocks when given, default is {MESSAGE_RECEIVE_MODE_BLOCKING}
 * @param-opt tx:               transaction id
