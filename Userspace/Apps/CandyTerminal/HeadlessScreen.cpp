...
void HeadlessScreen::scroll(int value)
{
	...
	if (value > 0)
	{
		for (int i = scrollEnd - 2; i >= scrollStart; i--)
			memcpy(&outputCurrent[(i + value) * SCREEN_WIDTH * 2], &outputCurrent[i * SCREEN_WIDTH * 2], SCREEN_WIDTH * 2);
	}

	else
	{
		for (int i = scrollStart; i < scrollEnd; i++)
			memcpy(&outputCurrent[i * SCREEN_WIDTH * 2], &outputCurrent[(i + value) * SCREEN_WIDTH * 2], SCREEN_WIDTH * 2);
	}
