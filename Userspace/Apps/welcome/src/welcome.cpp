...
void fillPresentation()
{
	...
	rg1[0] = "Hi " + Environment::get("USER") + " welcome to MeetiX OS " + Environment::get("VERSION");
	...
	rg2[0] = "I will guide you with a brief introduction to the main aspects of the graphical interface";
	if (style == UiStyle_t::KDE) rg2[1] = "The OsmosUI has a KDE style, so you can find a bar on the bottom of the screen,";
	else rg2[1] = "The OsmosUI has a GNOME style, so you can find a bar on the top of the screen,";
	...
	rg3[0] = "so you gain confidence with the system and applications; click the 'X' to exit whenever you want";
	rg3[1] = "Starting from the left, a menu with the applications list and real-time memory consumption indicator.";
	...
	rg4[1] = "At the center of the bar is the window name manager, which displays the names of running applications";
...
static void showPage(int index)
{
	if (index == 1)
	{
		...
		if (style == UiStyle_t::GNOME) backGround->setPNG("/app/welcome/deps/icon/arrow_up.png", Point(resolution.width / 2 - 53, 0));
		else backGround->setPNG("/app/welcome/deps/icon/arrow_down.png", Point(resolution.width / 2 - 53, resolution.height - 230));
	...
	{
		mainWindow->setBounds(windowDimension);
		if (style == UiStyle_t::GNOME) backGround->setPNG("/app/welcome/deps/icon/arrow_down.png", Point(resolution.width / 2 - 53, resolution.height - 300));
		lb1->setTitle(rg1[index]);
		lb2->setTitle(rg2[index]);
		lb3->setTitle(rg3[index]);
		lb4->setTitle(rg4[index]);
	...
	{
		mainWindow->setBounds(windowDimension);
		if (style == UiStyle_t::GNOME) backGround->setPNG("/app/welcome/deps/icon/arrow_down.png", Point(resolution.width / 2 - 200, resolution.height - 300));
		lb1->setTitle(rg1[index]);
		lb2->setTitle(rg2[index]);
		lb3->setTitle(rg3[index]);
		lb4->setTitle(rg4[index]);
...
int main(int argc, char *argv[])
{
	...
	if (UI::open() == UI_OPEN_STATUS_SUCCESSFUL)
	{
		...
		backGround->setBounds(Rectangle(0, style == UiStyle_t::KDE ? 0 : 30, resolution.width, resolution.height - (style == UiStyle_t::KDE ? 30 : 94)));
