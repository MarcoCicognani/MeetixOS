...
TEST_CASE(default_construction) {
    auto const set = Set<i32>::new_empty();
...
TEST_CASE(initializer_list) {
    auto const set = Set<StringView>::new_from_list({ "Hey"sv, "I'm"sv, "abc"sv });
...
TEST_CASE(copy_other) {
    ...
    auto const other_set = Set<StringView>::new_from_other(set);
...
TEST_CASE(insert) {
    auto set = Set<i32>::new_with_capacity(3);
...
TEST_CASE(iterate) {
    auto const set = Set<StringView>::new_from_list({ "One"sv, "Two"sv, "Three"sv });
    ...
    auto const ordered_set = OrderedSet<i32>::new_from_list({ 1, 2, 3, 4 });
...
TEST_CASE(ordered_set_reverse_iterator) {
    auto const ordered_set = OrderedSet<i32>::new_from_list({ 1, 2, 3, 4, 5 });
...
TEST_CASE(find) {
    auto const set = Set<StringView>::new_from_list({ "aa"sv, "bb"sv, "cc"sv, "dd"sv });
...
TEST_CASE(remove) {
    auto ordered_set = OrderedSet<StringView>::new_from_list({ "Hi"sv, "I'm"sv, "Marco"sv });
