...
static const AJS_HeapConfig ajsheap_config[] = {
    ...
    { 400,
      1,
      AJS_POOL_BORROW,
      0 }, /* duk_hthread, with heap ptr compression, RAM strings+objects */
...
#    if defined(DUK__ROMPTR_COMPRESSION)
    if ( ret >= DUK__ROMPTR_FIRST ) {
        fprintf(stderr,
                "Failed to compress pointer, in 16-bit range but matches romptr range: %p (ret was "
                "%ld)\n",
                (void*)p,
                (long)ret);
        fflush(stderr);
...
const void* ajsheap_extstr_check_1(const void* ptr, duk_size_t len) {
    ...
    for ( p = ajsheap_strdata, p_end = p + ajsheap_strdata_used; p != p_end; p++ ) {
        ...
        left = (size_t)(p_end - p);
        if ( left >= len + 1 && memcmp(p, ptr, len) == 0 && p[len] == 0 ) {
            ret = p;
#    if 0
