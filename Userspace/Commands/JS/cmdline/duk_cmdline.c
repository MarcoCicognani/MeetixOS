...
static const char* linenoise_hints_script
    = "(function linenoiseHints(input) {\n"
      ...
      "            found[name] = true;\n"
      "            res.push(res.length === 0 ? name.substring(partial.length) : (first ? ' || ' : "
      "' | ') + name);\n"
      "            first = false;\n"
      "        }\n"
      "        obj = Object.getPrototypeOf(Object(obj));\n"
...
static int
handle_fh(duk_context* ctx, FILE* f, const char* filename, const char* bytecode_filename) {
    char*  buf = NULL;
    size_t bufsz;
    size_t bufoff;
    size_t got;
    ...
    for ( ;; ) {
        ...
        if ( bufoff >= 16384 ) {
            ...
            for ( i = 16; i < bufoff / 8; i++ ) {
                ...
                if ( ok ) {
                    fprintf(stderr,
                            "emscripten workaround: detect looping at index %ld, verified with %ld "
                            "loops\n",
                            (long)i,
                            (long)(nloops - 1));
                    bufoff = i;
...
static duk_context* create_duktape_heap(int alloc_provider, int debugger, int ajsheap_log) {
    ...
    if ( !ctx && alloc_provider == ALLOC_AJSHEAP ) {
        ...
        ctx = duk_create_heap(ajsheap_log ? ajsheap_alloc_wrapped : AJS_Alloc,
    ...
    if ( debugger ) {
        ...
        duk_trans_socket_waitconn();
        fprintf(stderr,
                "Debugger connected, call duk_debugger_attach() and then execute requested "
                "file(s)/eval\n");
        fflush(stderr);
        duk_debugger_attach(ctx,
                            duk_trans_socket_read_cb,
...
int main(int argc, char* argv[]) {
    ...
    EM_ASM(
        ...
        */
        ...
        } catch ( e ) { console.log("Failed to mount NODEFS /working: " + e); }
        ...
        */
        try { FS.mount(NODEFS, { root : "/tmp" }, "/tmp"); } catch ( e ) {
...
usage:
    fprintf(stderr,
            ...
            "\n"
            "   -i                 enter interactive mode after executing argument file(s) / eval "
            "code\n"
            "   -e CODE            evaluate code\n"
            "   -c FILE            compile into bytecode (use with only one file argument)\n"
            "   --run-stdin        treat stdin like a file, i.e. compile full input (not line by "
            "line)\n"
