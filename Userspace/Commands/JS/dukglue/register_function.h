...
// (inconsistent 2-3% performance improvement for a 10,000 function call stress test averaged over
// 100 runs), since it has much uglier syntax and may bloat executable size if you have many
...
void dukglue_register_function_compiletime(duk_context* ctx, RetType (*)(Ts...), const char* name) {
    static_assert(
        std::is_same<T, RetType(Ts...)>::value,
    ...
    duk_c_function evalFunc
        = dukglue::detail::FuncInfoHolder<RetType,
                                          Ts...>::template FuncActual<Value>::call_native_function;
...
void dukglue_register_function(duk_context* ctx, RetType (*funcToCall)(Ts...), const char* name) {
    duk_c_function evalFunc
        = dukglue::detail::FuncInfoHolder<RetType, Ts...>::FuncRuntime::call_native_function;
