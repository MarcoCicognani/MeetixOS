...
//      a pointer (just returns script_object.\xFFobj_ptr), or a reference (dereferences
//      script_object.\xFFobj_ptr if it is not null).
...
//      Attempting to read a pointer (T*) or non-const reference (T&) will give a compile-time
//      error. You can also use this to implement your own lightweight types, such as a 3D vector.
//      (Strictly speaking, non-const references (T&) *could* be returned, but any changes to the
//      reference would be discarded. So, I wrote a static assert to disable the option. If you
//      understand the implications, you should be able to safely comment out the static_assert in
//      ArgStorage.)
template<typename T>
struct DukType {
    static_assert(std::is_same<T, typename Bare<T>::type>::value,
                  "Invalid base type, expected bare type");

    typedef std::false_type IsValueType;

    // read pointer
    template<typename FullT,
             typename = typename std::enable_if<std::is_pointer<FullT>::value>::type>
    static T* read(duk_context* ctx, duk_idx_t arg_idx) {
        ...
        if ( !duk_is_pointer(ctx, -1) ) // missing type_info, must not be a native object
            duk_error(ctx,
                      DUK_RET_TYPE_ERROR,
                      "Argument %d: expected native object (missing type_info)",
                      arg_idx);
    ...
    // read reference
    template<typename FullT,
             typename = typename std::enable_if<std::is_reference<FullT>::value>::type>
    static T& read(duk_context* ctx, duk_idx_t arg_idx) {
        T* obj = read<T*>(ctx, arg_idx);
        if ( obj == nullptr )
            duk_error(ctx,
                      DUK_RET_TYPE_ERROR,
                      "Argument %d: cannot be null (native function expects reference)",
                      arg_idx);
    ...
    /*template<typename FullT, typename = typename std::enable_if< std::is_same<T, typename
    std::remove_const<FullT>::type >::value>::type > static T read(duk_context* ctx, duk_idx_t
    arg_idx) { static_assert(std::is_copy_constructible<T>::value, "Reading a value requires a
    copy-constructable type"); const T& obj = read<T&>(ctx, arg_idx); return T(obj);
    }*/
    ...
    /*template<typename FullT, typename = typename std::enable_if< std::is_same<T, typename
    std::remove_const<FullT>::type >::value>::type > static void push(duk_context* ctx, T value) {
            static_assert(std::is_copy_constructible<T>::value, "Cannot push value for
    non-copy-constructable type."); return push<T*>(ctx, new T(value));
    }*/
};
...
// Native objects are already allocated on the heap, so there's no problem storing, say, const Dog&
// in the tuple.
template<typename T>
struct ArgStorage {
private:
    ...
    static_assert(!IsValueType::value || !std::is_pointer<T>::value,
                  "Cannot return pointer to value type.");
