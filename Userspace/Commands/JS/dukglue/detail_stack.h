...
get_stack_values_helper(duk_context* ctx, dukglue::detail::index_tuple<Indexes...>) {
    using namespace dukglue::types;
    return std::forward_as_tuple(
        DukType<typename Bare<Args>::type>::template read<typename ArgStorage<Args>::type>(
            ctx,
            Indexes)...);
}
...
typename ArgsTuple<Args...>::type get_stack_values(duk_context* ctx) {
    ...
    // So, we use traits::make_indexes<Ts...>, which returns a traits::index_tuple<0, 1, 2, ...>
    // object. We pass that into a helper function so we can put a name to that <0, 1, ...> template
    // argument. Here, the type of Args isn't important, the length of it is.
