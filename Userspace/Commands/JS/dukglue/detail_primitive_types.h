...
DUKGLUE_SIMPLE_VALUE_TYPE(uint64_t,
                          duk_is_number,
                          duk_get_number,
                          duk_push_number,
                          value) // have to cast to double
...
DUKGLUE_SIMPLE_VALUE_TYPE(int64_t,
                          duk_is_number,
                          duk_get_number,
                          duk_push_number,
                          value) // have to cast to double
...
DUKGLUE_SIMPLE_VALUE_TYPE(std::string,
                          duk_is_string,
                          duk_get_string,
                          duk_push_string,
                          value.c_str())
...
// char* is a bit tricky because its "bare type" should still be const char*, to differentiate it
// from just char
template<>
struct Bare<char*> {
...
struct DukType<std::shared_ptr<T>> {
    ...
    static std::shared_ptr<T> read(duk_context* ctx, duk_idx_t arg_idx) {
        ...
        if ( !duk_is_pointer(ctx, -1) ) // missing type_info, must not be a native object
            duk_error(ctx,
                      DUK_RET_TYPE_ERROR,
                      "Argument %d: expected shared_ptr object (missing type_info)",
                      arg_idx);

        // make sure this object can be safely returned as a T*
        dukglue::detail::TypeInfo* info
            = static_cast<dukglue::detail::TypeInfo*>(duk_get_pointer(ctx, -1));
        if ( !info->can_cast<T>() )
            duk_error(ctx,
                      DUK_RET_TYPE_ERROR,
                      "Argument %d: wrong type of shared_ptr object",
                      arg_idx);
        ...
        if ( !duk_is_pointer(ctx, -1) )
            duk_error(ctx,
                      DUK_RET_TYPE_ERROR,
                      "Argument %d: not a shared_ptr object (missing shared_ptr)",
                      arg_idx);
...
struct DukType< std::function<RetT(ArgTs...)> > {
        ...
        static void push(duk_context* ctx, std::function<RetT(ArgTs...)> value) {
                static_assert(false, "Pushing an std::function has not been implemented yet.
Sorry!");
        }
};*/
} // namespace types
