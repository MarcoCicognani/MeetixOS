...
// This class is not really dependant on the rest of dukglue, but the rest of dukglue is integrated
// to support it. Script objects are persisted by copying a reference to the object into an array in
// the heap stash. When we need to push a reference to the object, we just look up that reference in
// the stash.

// DukValues can be copied freely. We use reference counting behind the scenes to keep track of when
// we need to remove our reference from the heap stash. Memory for reference counting is only
// allocated once a DukValue is copied (either by copy constructor or operator=). std::move can be
// used if you are trying to avoid ref counting for some reason.

// One script object can have multiple, completely separate DukValues "pointing" to it - in this
// case, there will be multiple entries in the "ref array" that point to the same object. This will
// happen if the same script object is put on the stack and turned into a DukValue multiple times
// independently (copy-constructing/operator=-ing DukValues will not do this!). This is okay, as we
// are only keeping track of these objects to prevent garbage collection (and access them later).
// This could be changed to use a map structure to look up one canonical entry per script object in
// the "ref array" (I guess it would be more like a ref map in this case), but this would require a
// map lookup every time we construct a DukValue. The performance difference probably isn't *that*
// noticeable (a good map would probably be amortized constant-time lookup), but I am guessing
// constructing many separate DukValues that point to the same script object isn't a very common
// thing.
class DukValue {
public:
    enum Type : uint8_t
    ...
    static DukValue copy_from_stack(duk_context* ctx, duk_idx_t idx = -1) {
        ...
        switch ( value.mType ) {
            ...
            default:
                throw DukException()
                    << "Cannot turn type into DukValue (" << value.type_name() << ")";
        }
...
public:
    ...
    inline void push() const {
        ...
        switch ( mType ) {
            ...
            default:
                throw DukException()
                    << "DukValue.push() not implemented for type (" << type_name() << ")";
        }
    }
    ...
    // - The returned value is binary and will not behave well if you treat it like a string (it
    // will almost certainly contain '\0').
    //   If you need to transport it like a string, maybe try encoding it as base64.
    // - Strings longer than 2^32 (UINT32_MAX) characters will throw an exception. You can raise
    // this to be a uint64_t if you need
    //   really long strings for some reason (be sure to change DukValue::deserialize() as well).
    // - Objects are encoded to JSON and then sent like a string. If your object can't be encoded as
    // JSON (i.e. it's a function),
    //   this will not work. This can be done, but I chose not to because it poses a security issue
    //   if you deserializing untrusted data. If you require this functionality, you'll have to add
    //   it yourself with using duk_dump_function(...).
    static_assert(sizeof(char) == 1, "Serialization probably broke");
...
private:
    ...
    union ValueTypes {
        bool   boolean;
        double number;
        void*  pointer; // if mType == NULLREF, this is 0 (otherwise holds pointer value when mType
                        // == POINTER)
