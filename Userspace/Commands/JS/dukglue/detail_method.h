...
struct MethodInfo {
    typedef typename std::conditional<isConst,
                                      RetType (Cls::*)(Ts...) const,
                                      RetType (Cls::*)(Ts...)>::type MethodType;

    // The size of a method pointer is not guaranteed to be the same size as a function pointer.
    // This means we can't just use duk_push_pointer(ctx, &MyClass::method) to store the method at
    // run time. To get around this, we wrap the method pointer in a MethodHolder (on the heap), and
    // push a pointer to that. The MethodHolder is cleaned up by the finalizer.
    struct MethodHolder {
        MethodType method;
    };
    ...
    struct MethodCompiletime {
        ...
        actually_call(duk_context* ctx, Cls* obj, const std::tuple<BakedTs...>& args) {
            ...
            RetType return_val
                = dukglue::detail::apply_method<Cls, RetType, Ts...>(methodToCall, obj, args);

            using namespace dukglue::types;
            DukType<typename Bare<RetType>::type>::template push<RetType>(ctx,
                                                                          std::move(return_val));
        }
    ...
    struct MethodRuntime {
        ...
        template<typename Dummy = RetType, typename... BakedTs>
        static typename std::enable_if<!std::is_void<Dummy>::value>::type
        actually_call(duk_context*                  ctx,
                      MethodType                    method,
                      Cls*                          obj,
                      const std::tuple<BakedTs...>& args) {
            // ArgStorage has some static_asserts in it that validate value types,
            // so we typedef it to force ArgStorage<RetType> to compile and run the asserts
            typedef typename dukglue::types::ArgStorage<RetType>::type ValidateReturnType;

            RetType return_val
                = dukglue::detail::apply_method<Cls, RetType, Ts...>(method, obj, args);
...
struct MethodVariadicRuntime {
    ...
    static duk_ret_t call_native_method(duk_context* ctx) {
        ...
        // (should always be valid unless someone is intentionally messing with this.obj_ptr...)
        Cls*                  obj = static_cast<Cls*>(obj_void);
        MethodHolderVariadic* method_holder
            = static_cast<MethodHolderVariadic*>(method_holder_void);
