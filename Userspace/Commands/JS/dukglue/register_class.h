...
void dukglue_set_base_class(duk_context* ctx) {
    static_assert(!std::is_pointer<Base>::value && !std::is_pointer<Derived>::value
                      && !std::is_const<Base>::value && !std::is_const<Derived>::value,
                  "Use bare class names.");
...
void dukglue_register_method_compiletime(duk_context* ctx,
                                         RetType (Cls::*method)(Ts...),
                                         const char* name) {
...
template<typename T, T Value, class Cls, typename RetType, typename... Ts>
void dukglue_register_method_compiletime(duk_context* ctx,
                                         RetType (Cls::*method)(Ts...) const,
                                         const char* name) {
    static_assert(std::is_same<T, RetType (Cls::*)(Ts...) const>::value,
                  "Mismatching method types.");
...
template<class Cls, typename RetType, typename... Ts>
void dukglue_register_method(duk_context* ctx,
                             RetType (Cls::*method)(Ts...) const,
                             const char* name) {
...
void dukglue_register_method(
    duk_context* ctx,
    typename std::conditional<isConst, RetType (Cls::*)(Ts...) const, RetType (Cls::*)(Ts...)>::type
                method,
    const char* name) {
    using namespace dukglue::detail;
    typedef MethodInfo<isConst, Cls, RetType, Ts...> MethodInfo;
    ...
    duk_put_prop_string(ctx,
                        -2,
                        "\xFF"
                        "method_holder"); // consumes raw method pointer
...
template<class Cls>
inline void dukglue_register_method_varargs(duk_context* ctx,
                                            duk_ret_t (Cls::*method)(duk_context*),
                                            const char* name) {
...
template<class Cls>
inline void dukglue_register_method_varargs(duk_context* ctx,
                                            duk_ret_t (Cls::*method)(duk_context*) const,
                                            const char* name) {
    dukglue_register_method_varargs<true, Cls>(ctx, method, name);
}

template<bool isConst, typename Cls>
void dukglue_register_method_varargs(
    duk_context*                                                      ctx,
    typename std::conditional<isConst,
                              duk_ret_t (Cls::*)(duk_context*) const,
