...
int usage(const char* cmdname) {
    ...
    println(
        "\t--stack         [test the memory stack with recursive function and big local array]");
...
int main(int argc, const char* argv[]) {
    ...
    struct option longopts[]
        = { { "heap", no_argument, 0, 'd' },   { "stack", no_argument, 0, 's' },
            { "thread", no_argument, 0, 't' }, { "pthread", no_argument, 0, 'p' },
            { "fs", no_argument, 0, 'f' },     { 0, 0, 0, 0 } };
    ...
    while ( (opt = getopt_long(argc, argv, "vh?", longopts, NULL)) != EOF ) {
        switch ( opt ) {
            ...
            case 's':
                return testStack(verbose);
            case 't':
                return testThread(verbose);
            case 'p':
                return testPosixThread(verbose);
            case 'f':
                return testFilesystem(verbose);
            case 'v':
                verbose = true;
            case 'h':
                return usage(argv[0]);
