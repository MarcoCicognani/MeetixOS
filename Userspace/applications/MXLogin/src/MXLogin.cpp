...
static void loginToMeetiX()
{
	...
	if (mode == LOGIN) username = usernameField->getTitle();
	else if (mode == LOCK) username = usernameLabel->getTitle();
	...
	else
	{
		...
		if (mode == LOGIN) usernameField->setTitle("");
...
static bool researchAccess(string username, string password, LoginMode_t mode)
{
	...
	for (pair<string, string> current : properties)
	{
		...
		{
			if (mode != LoginMode_t::LOCK) Environment::set("USER", username);
			return true;
		}
	}
...
void initializeObjects()
{
	...
	if (mode == LOGIN) usernameField = Textfield::create();
	else if (mode == LOCK) usernameLabel = Label::create();
...
void paintAnimation()
{
	...
	for (int yPosition = 180; yPosition >= 100; yPosition--)
	{
		...
		if (!(yPosition % 2)) animation->setBounds(Rectangle(0, yPosition, resolution.width, 200));
...
int main(int argc, char *argv[])
{
	...
	if (UI::open() == UI_OPEN_STATUS_SUCCESSFUL)
	{
		...
		if (argc == 2 && !strcmp(argv[1], "lock")) mode = LOCK;
		...
		if (mode == LOGIN) loginRectangle->addChild(usernameField);
		else if (mode == LOCK) loginRectangle->addChild(usernameLabel);
