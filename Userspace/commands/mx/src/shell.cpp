	...
	while (true)
	{
		...
		else if (c == SHELLKEY_LEFT)
		{
			if (caret > 0)
			{
				...
				caret--;
				if (beforeCaret == '\t') Shell::moveCursorBack(4);
				else Shell::moveCursorBack(1);
			}
		}
		...
		else if (c == SHELLKEY_RIGHT)
		{
			if (caret < line.size())
			{
				...
				caret++;
				if (atCaret == '\t') Shell::moveCursorForward(4);
				else Shell::moveCursorForward(1);
			}
		}
...
bool handleBuiltin(std::string cwd, ProgramCall *call)
{
	if (call->program == "cd")
	{
		...
		else std::cerr << "Usage:\tcd /path/to/target" << std::endl;
		return true;
	}
	...
	{
		if (call->arguments.size() == 1) std::cout << findProgram(cwd, call->arguments.at(0)) << std::endl;
		else std::cerr << "Usage:\twhich command" << std::endl;
		return true;
	}
...
void MXShell::shellMode(Environment *env)
{
	...
	while (true)
	{
		...
		if (GetWorkingDirectory(cwdbuf) == GET_WORKING_DIRECTORY_SUCCESSFUL) std::cout << (char) 27 << "[35m" << cwdbuf << (char) 27 << "[0m";
		else std::cout << "?";
		...
		if (!cmdparser.pipeExpression(&pipeexpr)) continue;
		...
		for (int c = 0; c < numCalls; c++)
		{
			...
			if (numCalls == 1 && c == 0 && handleBuiltin(cwd, call)) break;
			...
			for (auto arg : call->arguments)
			{
				...
				else argstream << (char) CLIARGS_SEPARATOR;
				argstream << arg;
			}
			...
			if (c == 0) inStdio[0] = STDIN_FILENO;
			else inStdio[0] = previousOutPipeR;

			// IDstdout must be chosen
			if ((numCalls == 1 && c == 0) || c == numCalls - 1) inStdio[1] = STDOUT_FILENO;
			else inStdio[1] = outPipeW;

			// do spawning
			Pid outPid;
			File_t outStdio[3];
			SpawnStatus status = SpawnPOI(findProgram(cwd, call->program).c_str(), argstream.str().c_str(), cwdbuf, SECURITY_LEVEL_APPLICATION, &outPid, outStdio, inStdio);

			// check result
			if (status == SPAWN_STATUS_SUCCESSFUL)
			{
				if (firstProcessID == -1) firstProcessID = outPid;
			...
			else
			{
				...
				if (status == SPAWN_STATUS_FORMAT_ERROR) std::cout << call->program << ": invalid binary format" << std::endl;
				else if (status == SPAWN_STATUS_IO_ERROR) std::cout << call->program << ": command not found" << std::endl;
				else std::cout << call->program << ": unknown error during program execution" << std::endl;
				std::cout << (char) 27 << "[0m";
				std::flush(std::cout);
				break;
			}
