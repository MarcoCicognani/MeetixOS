...
void init() {
  ...
  Pid mxPid;
  SpawnStatus stat = spawn("/cmd/mx", "-s inittab.sh", "/cfg/init/bsp",
                           SECURITY_LEVEL_KERNEL, 0, &mxPid, 0, 0, 0, 0, 0, 0);
  ...
  if (stat == SPAWN_STATUS_SUCCESSFUL) {
    ...
    klog("failed to load mx shell from '/bin/mx' with code %d", stat);
}
...
void receiveRequests() {
  ...
  const size_t requestLenMax =
      sizeof(MessageHeader) + sizeof(SpawnCommandSpawnRequest) + 1024;
  ...
  while (true) {
    ...
    SpawnCommandHeader *commandHeader =
        (SpawnCommandHeader *)MESSAGE_CONTENT(header);
    ...
    if (commandHeader->command == SPAWN_COMMAND_SPAWN_REQUEST)
      ...
      protocolError("received unknown command: code %i, task %i",
                    commandHeader->command, header->sender);
  }
}
 ...
 *
 */
void processSpawnRequest(SpawnCommandSpawnRequest *request, Tid requester,
  ...
  SpawnStatus spawnStatus =
      spawn(path, args, workdir, secLvl, requesterPid, &oPid, &oFdInw, &oFdOutr,
            &oFdErr, request->stdin, request->stdout, request->stderr);
 ...
 *
 */
bool createPipe(Pid thisPid, Pid requesterPid, Pid targetPid, File_t source,
                File_t *out, File_t target) {
 ...
 *
 */
bool setupStdio(Pid createdPid, Pid requesterPid, File_t *outStdin,
                File_t *outStdout, File_t *outStderr, File_t inStdin,
                File_t inStdout, File_t inStderr) {
  uint32_t thisPid = GetPid();
  if (!createPipe(thisPid, requesterPid, createdPid, inStdin, outStdin,
                  STDIN_FILENO))
    return false;
  if (!createPipe(thisPid, requesterPid, createdPid, inStdout, outStdout,
                  STDOUT_FILENO))
    return false;
  if (!createPipe(thisPid, requesterPid, createdPid, inStderr, outStderr,
                  STDERR_FILENO))
    return false;
...
bool findFile(const char *path, File_t *file) {
  ...
  std::vector<std::string> paths =
      Arguments::split(Environment::get("PATH"), ':');
 ...
 *
 */
SpawnStatus spawn(const char *path, const char *args, const char *workdir,
                  SecurityLevel secLvl, Pid requesterPid, Pid *outPid,
                  File_t *outStdin, File_t *outStdout, File_t *outStderr,
                  File_t inStdin, File_t inStdout, File_t inStderr) {
  // file of executable
  File_t file;

  // open input file
  if (!findFile(path, &file)) {
    klog("unable to open the file: %s, it doesn't exist on PATH directory",
         path);
  ...
  // setup standard I/O
  if (!setupStdio(targetPid, requesterPid, outStdin, outStdout, outStderr,
                  inStdin, inStdout, inStderr))
