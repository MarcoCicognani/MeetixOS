...
void Cmos_t::readRTC() {
    ...
    if ( centuryRegister != 0 )
        var.century = getRTCregister(centuryRegister);
    ...
    } while ( (last_second == var.second) && (last_minute == var.minute) && (last_hour == var.hour)
              && (last_day == var.day) && (last_month == var.month) && (last_year == var.year)
              && (last_century == var.century) );
    ...
    if ( !(registerB & 0x04) ) {
        ...
        if ( centuryRegister != 0 )
            var.century = (var.century & 0x0F) + ((var.century / 16) * 10);
    }
    ...
    if ( centuryRegister != 0 )
        var.year = var.year + var.century * 100;

    else {
        var.year = var.year + (currentYear / 100) * 100;

        if ( var.year < currentYear )
            var.year = var.year + 100;
    }
}
