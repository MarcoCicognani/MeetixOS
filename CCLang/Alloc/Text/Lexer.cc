...
auto Lexer::consume_until(char c) -> StringView {
    usize const start_index = m_index;
...
auto Lexer::ignore(usize count) -> void {
    m_index += usize::min(count, m_source_view.len() - m_index);
}

auto Lexer::ignore_until(char stop_c) -> void {
...
auto Lexer::ignore_until(StringView stop_word) -> void {
