...
auto u8::operator=(u8&& rhs) -> u8& {
   auto integer = Cxx::forward<u8>(rhs);
   swap(integer);
   return *this;
}
...
auto u8::try_set_bit(usize index, bool value) -> ErrorOr<void> {
   if ( index > bit_count() ) {
       return Error::from_code(ErrorCode::IndexOutOfRange);
   }
...
auto u8::set_bit(usize index, bool value) -> void {
...
auto u8::try_left_shift(u8 const& rhs) const -> ErrorOr<u8> {
   if ( rhs >= bit_count().as<u8>() ) {
       ...
       return u8(static_cast<CCIntegerType>(m_value << rhs.m_value));
   }
}
