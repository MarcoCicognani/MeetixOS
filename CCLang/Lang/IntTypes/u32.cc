...
auto u32::ceil_div(u32 const& lhs, u32 const& rhs) -> u32 {
    u32 res = lhs / rhs;
    if ( (lhs % rhs) != 0 ) {
        res += 1;
    }
...
auto u32::try_set_bit(usize index, bool bit_value) -> ErrorOr<void> {
    if ( index > bit_count() ) {
        return Error::from_code(ErrorCode::IndexOutOfRange);
    }
...
auto u32::set_bit(usize index, bool bit_value) -> void {
...
auto u32::try_left_shift(u32 const& rhs) const -> ErrorOr<u32> {
    if ( rhs >= bit_count().as<u32>() ) {
        ...
        return u32(static_cast<NativeInt>(m_value << rhs.m_value));
    }
}
